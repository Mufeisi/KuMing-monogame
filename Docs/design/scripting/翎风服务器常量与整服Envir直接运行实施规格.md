...
- 封神原版 `法宝收录.txt` 与 `法宝图鉴.csv` 已使用原数据库物品完成扣币、回收、收录、保存和重启；`HCALL` 已接通指定在线人物与唯一原页面，原版三参数 `HUMANMP` 也进入真实资源调整链。但完整 `@属性计算` 仍被动态数据库字段语法长尾阻断，同一原版流程尚未覆盖 GATE-E2 要求的登录、NPC、怪物、完整奖励与 Android 显示。合成切片只能验证接缝，不能和该局部流程拼接为完整玩法。
