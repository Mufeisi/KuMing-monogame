...
| 16 | LFENV-16 单版本完整切片 | 阻断 | `01酷明命格` 与 `封神法宝` 两条独立迁移证据 | 酷明原样 Envir E1 通过；酷明只以命格验证，封神只以法宝验证，两套玩法不混用；至少一个完整玩法 E2 通过 |
| 17 | LFENV-17 多版本回归 | 进行中 | 代表版本矩阵、差异报告 | 十个家族样本均达到声明等级 |
...
- 酷明真实资源审计以原样 Envir、源数据库和源地图为输入，只读取实际物品、怪物、编号和 `.map` 文件并评估外部依赖 manifest；缺失项结构化进入 `Missing`，不再创建占位名称、编号或替代地图。补齐真实物品、怪物和地图并完成生产冷启动前，LFENV-16 保持阻断。
