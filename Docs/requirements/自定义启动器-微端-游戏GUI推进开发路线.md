...
| 项目 | 内容 |
|---|---|
| 文档状态 | 历史路线与设计基线；启动器编辑器与独立微端 L0～L5 已完成；游戏 GUI 已由 [`LEG-07-自定义游戏GUI.md`](LEG-07-自定义游戏GUI.md) 正式激活 |
| 日期 | 2026-08-12 |
| 适用仓库 | LyoCrystal |
...
> 生命周期说明：本文保留阶段设计与先后关系，不直接承担动态任务状态。已实施边界见《启动器编辑器与独立微端实施规格》及 `Docs/Evidence/GATE-L0..L5/`；当前游戏 GUI 的范围、状态与验收只以[`LEG-07-自定义游戏GUI.md`](LEG-07-自定义游戏GUI.md)为事实源。
