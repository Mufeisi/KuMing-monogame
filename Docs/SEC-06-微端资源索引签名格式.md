...
每端在各自应用私有运行时目录原子保存 `BootstrapManifestSecurityState.json`，记录最高已接受 `Sequence`、`ResourceVersion`、`KeyId`、规范载荷 SHA-256 和原始已签名清单；首次接受时另写安装标记。以后加载状态会重新验证保存清单的签名与载荷摘要：
