...
每端在各自应用私有运行时目录原子保存 `BootstrapManifestSecurityState.json`，记录最高已接受 `Sequence`、`ResourceVersion`、`KeyId`、规范载荷 SHA-256 和原始已签名清单；安装标记另存并绑定最高 `Sequence + 规范载荷 SHA-256`。以后加载状态会重新验证保存清单的签名、载荷摘要和标记版本地板：

- 低于最高序列：拒绝。
- 等于最高序列但资源版本不同：拒绝。
- 等于最高序列、资源版本和规范载荷摘要均相同：允许幂等重试；同序列替换任一签名字段或包集合均拒绝。
- 防降级状态存在但损坏：失败关闭，不当作首次安装。
- 当前安装已有标记但防降级状态被单独删除：失败关闭，不当作首次安装。
- 把状态替换为仍有有效签名的旧快照：因低于标记版本地板而失败关闭；状态已原子前推但标记仍旧时，验证新状态签名后安全前推标记，以容忍双文件更新之间崩溃。
- 更新队列的资源版本及每个包名/SHA-256 必须逐项匹配保存并重新验签的清单；旧队列或被篡改队列忽略，不继续下载。
- 来自签名队列的 ZIP 必须使用清单内 SHA-256 校验，PC 和 Mono/Android 的旧可选校验开关都不能关闭该检查；未签名旧路径才可使用可选 `.zip.sha256`。
- 签名或状态验证失败：更新计划返回失败；不会把远端内容缓存成已接受索引，也不会生成更新队列。

卸载/清除应用数据会同时清除本地最高序列和安装标记，这是新安装边界；只删除其中一个文件不构成新安装。RELEASE 阶段若要求跨重装保持版本地板，应把最高序列接入平台受保护且可恢复的设备状态或在线发行策略。

PC 显式使用兼容版本 `1.0.0`，Mono/Android 显式使用与 Android 发行显示版本一致的 `2.0.0`；发行版本变化时必须同步调整对应宿主常量。
