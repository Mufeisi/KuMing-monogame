...
| G7 | 构建可复现 | 从指定源码提交 + 资源获取脚本后，可在受支持环境复现构建 |
...
| BASE-02 | **可复现构建**：`global.json` 锁定到可复现稳定 SDK；定义外部资源包/测试资源/地图/补丁资源的来源、版本、哈希、获取方式（README 声明资源在 QQ 群） | BASE-01 | 从指定提交 + 资源脚本后，受支持环境可复现构建 |
...
**GATE-P0 退出条件**：指定提交 + 资源脚本可复现构建；CI 矩阵全绿；net10 迁移完成；日志可查；最小测试集绿。**不满足不得进入 P1。**
...
| PERF-00 | 性能测量基建：加 CPU/Update/Draw/DrawCall/纹理切换/创建/GC/显存/保存耗时/网络队列统计；录制 3 个固定压力场景（含 300 连接/100 活跃口径） | GATE-P0 | 指标可采集，3 场景基线数据建立 |

**GATE-P1 退出条件**：Android 真机目标功能闭环；wire manifest 就绪；3 场景基线数据存在。
...
| 外部资源（QQ 群）不可复现 | 高 | 中 | P0 资源来源基线 + 哈希清单 |
