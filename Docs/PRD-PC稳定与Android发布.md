...
| GATE-P4 | **已完成**：自动压力测试按产品决定移出门禁；PERF-01/02/04 无基线不做猜测性改动，PERF-03 深模块与回归完成，PERF-05 口径定稿 | 无 |
| GATE-P5 | 未开始 | 按门禁顺序进入 GATE-P5 |
...
| PERF-00 | 性能测量基建：加 CPU/Update/Draw/DrawCall/纹理切换/创建/GC/显存/保存耗时/网络队列统计 | GATE-P0 | **已完成**；S1/S2/S3 自动压力场景已按 2026-08-10 产品决定移出门禁，见 `PERF-00性能指标与基线记录.md` |

**GATE-P1 退出条件**：Android 目标功能按 §2 的设备实操或替代使用探针闭环；wire manifest 就绪；集成测试与远程 CI 通过。PERF-00 的会话采集基建属于本门禁代码交付。用户于 2026-08-10 明确决定无需自动压力测试，后续由用户自行实机验证；S1/S2/S3 与 300 连接/100 活跃口径不再作为任一开发门禁，不得用合成数据冒充真实结果。
...
| 编号 | 任务 | 依赖 | 退出条件 |
|---|---|---|---|
| PERF-01 | 热点优化：**仅优化排名靠前且可复现的瓶颈**（ID/标准化精确名称可索引；模糊/语义查找不强行 O(1)） | GATE-P3/PERF-00 | **本轮不实施**：无排名证据，不做猜测性优化 |
| PERF-02 | 网络与 GC：**确认分配热点后**再用 ArrayPool/预分配（否则避免所有权/脏数据风险） | GATE-P3/PERF-00 | **本轮不实施**：无分配热点证据，不引入所有权风险 |
...
| PERF-05 | 稳定性验收口径定稿 | GATE-P3/PERF-00 | **已完成（产品决定）**：取消自动压力测试与数值阈值门禁；用户后续自行实机验证，不阻塞开发 |

**GATE-P4 退出条件（2026-08-10 产品决定）**：PERF-03 至少提取一个有真实接口的深模块并具备回归测试；专项、全量和相关平台构建通过。S1/S2/S3、300 连接/100 活跃、24/72h soak 与前后性能数值不再是开发门禁，用户后续实机验证不阻塞 GATE-P4/P5。

**PERF-03/GATE-P4 收口记录（2026-08-10）**：`HumanObject.RefreshStats` 原有普通套装、圣龙戒指、神龙部位与天龙组合规则已迁入纯进程内 `EquipmentSetBonusModule`；模块以 `ApplyItemSetBonuses`、`ApplyMirSetBonuses` 两个接口隐藏 263 行实现，巨型类移除原 267 行规则，不使用 `partial` 或假适配器。回归覆盖破碎、强青玉、圣龙左右戒、神龙局部/完整、天龙四层累积及原有提前结束语义；专项 6/6、Base05 全量 322/322、Server.Library 与 Server.MirForms Release 均 0 错误，证据见 `Docs/Evidence/GATE-P4/perf03-deep-module-20260810/`。按用户产品决定，PERF-01/02/04 不做无基线猜测性优化，PERF-05 取消自动压力数值门禁，GATE-P4 关闭并进入 GATE-P5。
...
**OPS-BASIC-04 收口记录（2026-08-10）**：使用 Microsoft SBOM Tool 4.1.5 为服务端、PC 和 Android 六个真实 Release 工件生成 SPDX 2.2 发布侧车，记录 218 个包、6 个文件及 988 条依赖关系；另将不含自引用文件哈希的 218 包依赖 SPDX、外部资源清单和许可证/NOTICE/EULA 正文随 PC、服务端、Android APK 与 iOS 发布。许可证自动识别 204/221 个唯一组件，12 个 `NOASSERTION` 项按包名和版本由专项测试锁定。初扫发现的 `log4net 3.0.3` 中危和 `SQLitePCLRaw.lib.e_sqlite3 2.1.11` 高危已分别升级到 3.3.2 和 2.1.12，Windows、Android、iOS 直接/传递依赖复扫原始 JSON 均为退出码 0、无漏洞包。`D:\ChuanQi\Crystal_monogame` 的素材、字体、FairyGUI、音频、地图和微端资源已按项目所有者的明确授权确认分类记录，不把 11GB 外部资源复制进 Git；资源签名继续复用 SEC-06。证据见 `Docs/Evidence/GATE-P5/ops-basic-04-license-audit-20260810/`；执行边界见 `Docs/OPS-BASIC-04-授权与依赖审计.md`。PROTO-02/03、RELEASE-01～03 及回补的 PERF-01/02 模拟并发压测仍未完成，GATE-P4/P5 保持开启。
...
## 5. 稳定性观测参考（不作为自动压力测试门禁）

> 2026-08-10 产品决定：自动压力测试和量化阈值不再作为开发门禁，后续由用户自行实机验证。下表保留为 PERF-00 可观测字段和发布后排障参考，不代表已完成 300 连接或 24/72h soak 验证。

| 指标 | 目标 | 说明 |
|---|---|---|
| 并发规模 | 300 连接 / 100 活跃角色（历史规划参考） | 非开发门禁，未声称已验证 |
