...
> 依据审计：不是"补齐所有未处理包"，而是"目标功能闭环通过"。状态分为代码/自动验证与真机验收两层，禁止用单元测试代替设备验收。按 2026-08-09 产品决定，本项目将逍遥模拟器作为 Android 实机等效设备；在逍遥上完成完整步骤即可标注“实机通过”，不再把缺少独立实体手机列为阻塞。
...
| 师徒（Mentor/Mentee） | 有 | 有 | 已实现 | 已实现 | 部分通过：入口和窗口；业务闭环待验收 | ANDROID-02 |
| 关系（Marriage 系） | 有 | 有 | 已实现 | 已实现 | 部分通过：入口和窗口；业务闭环待验收 | ANDROID-03 |
| 坐骑（Mount） | 有 | 有 | 已实现 | 已实现 | 窗口缺陷通过：逍遥确认不再误命中 `RankingGrideItem`，无坐骑资源时进入专用 fallback；完整业务闭环仍待验收 | ANDROID-04 |
| 物品封印/租赁 | 有 | 有 | 已实现 | 已实现 | 部分通过：入口和窗口；业务闭环待验收 | ANDROID-05 |
| 钓鱼 | 有 | 有 | 已实现 | 已实现 | 部分通过：入口和窗口；业务闭环待验收 | ANDROID-06 |
| 活动/赏金 | 有 | 有 | 已实现 | 已实现 | 窗口缺陷通过：逍遥确认活动 fallback、缓存双向切换与分页正常，不再触发已知构造异常；完整业务闭环仍待验收 | ANDROID-07 |
...
| GATE-P1 | ANDROID-01..07 代码闭环已实现；PROTO-01、BASE-10、PERF-00 证据已归档；本地服务端、Bootstrap 包、测试账号/角色和逍遥实机等效设备均已打通，已自动登录并加载 700×700 地图；商城商品渲染/购买端到端闭环已通过，坐骑误命中与活动窗口构造异常已关闭；师徒、关系、封印/租赁、钓鱼及坐骑/活动完整业务仍有验收缺口；**门禁未关闭** | 继续按独立任务完成剩余“请求→响应→状态→UI”闭环并归档证据 |
...
| P1-INTEGRATE | 进行中：上述三项已合并，集成 Base05 `217/217` 通过；等待剩余业务闭环与远程 CI | `Docs/Evidence/GATE-P1/integration/` 与本文 §4.3 | P1 成果全部合并，完整测试/远程 CI 绿，§4.3 快照更新 | 被剩余业务闭环阻塞；按文件所有权继续并行 |
...
| 编号 | 任务 | 依赖 | 退出条件 |
|---|---|---|---|
| ANDROID-01..07 | 功能闭环矩阵实现（商城→师徒→关系→坐骑→封印/租赁→钓鱼/活动） | GATE-P0 | **代码与自动化测试已实现；ANDROID-01 商城实机闭环通过，坐骑/活动窗口缺陷通过，其余真机业务矩阵待验收** |
