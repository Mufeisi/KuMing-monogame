...
| GATE-P1 | ANDROID-01..07 代码闭环已实现；PROTO-01、BASE-10、PERF-00 证据已归档；本地服务端、Bootstrap 包、测试账号/角色和逍遥实机等效设备均已打通，已自动登录并加载 700×700 地图；商城入口/窗口/15 格已通过，其他功能仍有部分闭环缺口；**门禁未关闭** | 修正坐骑误命中和活动窗口异常，逐项完成七项“请求→响应→状态→UI”闭环并归档证据 |
...
| P1-RUNTIME | 待审核：分支 `codex/p1-runtime-20260809`，商城 15 格适配已通过逍遥运行验证 | `Client_MonoGame.Shared/UI/FairyGui/`、Bootstrap 默认配置、`Docs/Evidence/GATE-P1/runtime-20260809/` | 本地服务、资源、账号、APK、设备证据归档；发现的真实缺口转为后续修正 | 独占移动 FairyGUI 运行时文件；其他会话不得重复领取 |
