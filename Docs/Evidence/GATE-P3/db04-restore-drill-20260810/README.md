...
真实演练使用 DB-03 正式备份服务生成 C: 本地与 D: 异卷副本；强停进程 PID、UTC 时间、三文件状态、SHA-256、正式 CLI 输出/退出码、五域读取值和 `Ready` 状态均在 `raw-powershell-transcript.txt` 中。可复算结果：RPO 303ms；恢复命令到 Ready 的 UTC 端点差为 602.31ms，保守上取整报告 RTO 603ms（进程内 Stopwatch 为 600ms）；故障到 Ready 625ms。均满足 RPO≤5 分钟、RTO≤30 分钟。
...
- 恢复：正式 Release `Server.dll` 返回 0，报告恢复耗时 101ms；五域复读、完整性检查及宿主 `Ready` 完成后，UTC 端点差 602.31ms，按 603ms 报告。
