...
`SKILL-06` 的物理资源外环发现 `M51-1.wav` 与 `M51-2.wav` 不存在，因此本页早期记录的音效 `+0/1/2` 仅代表当时的代码一致性，不再代表最终有效资源契约。最终双端契约已更正为施法播放 `M51-0.wav`，飞行与命中无独立音效；以 [`SKILL-06.md`](SKILL-06.md) 为准。
