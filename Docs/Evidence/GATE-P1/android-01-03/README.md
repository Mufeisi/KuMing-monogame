...
- 基本启动、登录页显示、前台→HOME→同一 Activity 恢复：在明确采集窗口内完成（见 `02-login.png`、`07-before-home.png`、`08-home.png`、`09-after-resume.png` 与 `foreground-home-flow.txt`）。
...
- `raw-logcat-20260809-0041.txt` 覆盖该采集窗口的全缓冲区原始输出；**仅就该窗口**未见应用 `FATAL EXCEPTION` 或立即崩溃，不作全局稳定性结论。
...
- `02-login-ui.xml`：启动后 Android UI 层级（根下为 `android.view.View`，业务控件由 MonoGame 绘制）。
- `07-before-home.png`：正确组件启动后、HOME 前的登录页截图。
- `08-home.png`：HOME 后的模拟器 Launcher 截图。
- `09-after-resume.png`：不 force-stop、使用同一组件恢复后的登录页截图。
- `03-login-resume.png`、`03-login-resume-ui.xml`、`06-after-resume-ui.xml`：较早采集的补充截图/层级，不作为本次 HOME 结论的唯一依据。
- `foreground-home-flow.txt`：完整命令、时间戳及三次 `dumpsys` 状态。
- `raw-logcat-20260809-0041.txt`：该窗口结束后用 `adb logcat -b all -d -v threadtime` 导出的完整原始日志。
