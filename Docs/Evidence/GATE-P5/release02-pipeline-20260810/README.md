...
- `shared-build.log`、`mono-shared-build.log`、`mono-android-build.log`、`pc-client-build.log`：共享库、Mono 桌面目标、Android 正式目标和 PC 消费者的 Release 构建输出。
...
- `gateway-proof.txt`：实际 loopback HTTP 网关的 ClientId 分桶、5% 当前版本选择、真实下载、未鉴权 401、受保护令牌事件、旧版本事件拒绝与幂等指标状态。
...
- r3 一键运行当时 Base05 为 366/366、专项为 15/15；最终安全修复后 Base05 为 368/368、专项为 17/17。
...
- 最终发布网关 PID 14844 实际监听 `127.0.0.1:18443`；`proof-canary-9` 的稳定桶为 4，实际返回 r3，并从响应的 `ResourceRepositoryPath` 下载签名索引 40,851 字节；无 Bearer 返回 401，受保护令牌接受当前 r3 cohort，旧 r2 事件返回 400，重复 HealthyLaunch EventId 未重复累计。
- Mono/Android 最终实现把整版资源、版本快照和清空后的签名队列纳入同一文件事务，并在入事务前逐包核对当前签名摘要；成功重启读取与失败全量回滚均有专项覆盖。
