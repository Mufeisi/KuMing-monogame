...
- 发布网关支持单段 HTTP `Range`；HTTP.sys 在脚本前拒绝溢出数字为 400，网关对已解析但越界的范围返回 416；客户端下载保留 `.part` 后会以 206 响应继续传输。
...
- 最终正式 arm64 AOT+Trim APK SHA-256：`E8E0F27EA5477D1753323705A0FAEFD84C5517ED33FABA872F439AA3CBDA24E2`，AOT 107/107；该 APK 由实际生产 keystore 的恢复副本重新构建。
