...
- GET 与非 GET 管理请求统一经过来源、鉴权和审计；同值角色令牌按未配置失败关闭。
- `ADMIN_AUDIT` 通过不可静默丢弃的警告级接缝记录来源关联标识、方法、规范化动作、角色与鉴权结果，不记录 IP 明文、令牌、Authorization 或查询串。
...
真实 `HttpListener` 用例验证缺失令牌与错误令牌返回 401、操作员读取状态成功、操作员访问开户端点返回 403、管理员通过鉴权、未鉴权 POST 返回 401、已鉴权 POST 返回 405；同时刷新并读取真实 Logger 文件，确认审计落盘且无令牌/IP 明文。纯策略用例覆盖公网/通配地址、内网明文 HTTP、角色矩阵、同值令牌失败关闭、未配置状态、固定时间令牌比较入口及无秘密审计格式。
