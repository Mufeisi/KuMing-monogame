...
namespace Server.MirObjects
{
    public class PlayerObject : HumanObject
    {
        ...
        private readonly HashSet<string> _csharpActiveRegions = new HashSet<string>(StringComparer.Ordinal);
        private readonly HashSet<string> _csharpActiveRegionsScratch = new HashSet<string>(StringComparer.Ordinal);
        private long _csharpRegionsScriptVersion = -1;
        private Map _csharpRegionsLastMap;
        ...
        protected override void Moved()
        {
            ...
            if (!_csharpRegionsLastInitialized || _csharpRegionsLastMap == null)
            {
                _csharpRegionsLastInitialized = true;
                _csharpRegionsLastMap = CurrentMap;
                _csharpRegionsLastLocation = CurrentLocation;
                SyncCSharpActiveRegions(CurrentMap, CurrentLocation);
                return;
            }

            var moveReason = _csharpRegionsMoveReasonOverride;

            if (moveReason == Server.Scripting.PlayerRegionTransitionReason.Unknown)
            {
                if (_csharpRegionsLastMap == CurrentMap)
                {
                    var dx = Math.Abs(CurrentLocation.X - _csharpRegionsLastLocation.X);
                    var dy = Math.Abs(CurrentLocation.Y - _csharpRegionsLastLocation.Y);
                    moveReason = dx > 1 || dy > 1
                        ? Server.Scripting.PlayerRegionTransitionReason.Run
                        : Server.Scripting.PlayerRegionTransitionReason.Walk;
                }
                else
                {
                    moveReason = Server.Scripting.PlayerRegionTransitionReason.Walk;
                }
            }

            TryCSharpPlayerRegionTransition(_csharpRegionsLastMap, _csharpRegionsLastLocation, CurrentMap, CurrentLocation, moveReason);
        }

        public override int Pushed(MapObject pusher, MirDirection dir, int distance)
        {
            _csharpRegionsMoveReasonOverride = Server.Scripting.PlayerRegionTransitionReason.Pushed;

            try
            {
                return base.Pushed(pusher, dir, distance);
            }
            finally
            {
                _csharpRegionsMoveReasonOverride = Server.Scripting.PlayerRegionTransitionReason.Unknown;
            }
        }
        public override void Die()
        {
            ...
            var allowCSharpBefore = scriptsRuntimeActive && Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnPlayerDeathPenaltyBefore);
            var allowCSharpAfter = scriptsRuntimeActive && Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnPlayerDeathPenaltyAfter);

            var defaultDropMode = Server.Scripting.DeathPenaltyDropMode.None;
            if (PKPoints > 200)
                defaultDropMode = Server.Scripting.DeathPenaltyDropMode.Red;
            else if (!InSafeZone)
                defaultDropMode = Server.Scripting.DeathPenaltyDropMode.Normal;

            var killer = LastHitter;
            var requestedDropMode = defaultDropMode;
            var scriptDecision = Server.Scripting.ScriptHookDecision.Continue;

            if (allowCSharpBefore)
            {
                var req = new Server.Scripting.PlayerDeathPenaltyRequest(killer, requestedDropMode, PKPoints, InSafeZone);
                Envir.CSharpScripts.TryHandlePlayerDeathPenaltyBefore(this, req);

                killer = req.Killer;
                requestedDropMode = req.DropMode;
                scriptDecision = req.Decision;
            }
            else if (scriptsRuntimeActive && Settings.TxtScriptsLogDispatch)
            {
                MessageQueue.Enqueue($"[Scripts][Dispatch] DeathPenalty -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnPlayerDeathPenaltyBefore}）");
            }

            var executedLegacy = false;
            var executedDropMode = Server.Scripting.DeathPenaltyDropMode.None;

            if (scriptDecision == Server.Scripting.ScriptHookDecision.Continue)
            {
                switch (requestedDropMode)
                {
                    case Server.Scripting.DeathPenaltyDropMode.None:
                        executedDropMode = Server.Scripting.DeathPenaltyDropMode.None;
                        break;
                    case Server.Scripting.DeathPenaltyDropMode.Normal:
                        DeathDrop(killer);
                        executedLegacy = true;
                        executedDropMode = Server.Scripting.DeathPenaltyDropMode.Normal;
                        break;
                    case Server.Scripting.DeathPenaltyDropMode.Red:
                        RedDeathDrop(killer);
                        executedLegacy = true;
                        executedDropMode = Server.Scripting.DeathPenaltyDropMode.Red;
                        break;
                    default:
                        executedDropMode = Server.Scripting.DeathPenaltyDropMode.None;
                        break;
            ...
            else if (scriptDecision == Server.Scripting.ScriptHookDecision.Handled)
            {
                executedDropMode = requestedDropMode;
            }

            if (allowCSharpAfter)
            {
                Envir.CSharpScripts.TryHandlePlayerDeathPenaltyAfter(
                    this,
                    new Server.Scripting.PlayerDeathPenaltyResult(
                        killer,
                        requestedDropMode,
                        executedDropMode,
                        executedLegacy,
        ...
        public override void WinExp(uint amount, uint targetLevel = 0)
        {
            ...
            var expRateRequest = Server.Scripting.EconomyRateHooks.ResolveExperienceRate(this, amount, targetLevel, "kill");

            if (expRateRequest.Decision == Server.Scripting.ScriptHookDecision.Cancel)
            {
                if (!string.IsNullOrEmpty(expRateRequest.FailMessage))
                    ReceiveChat(expRateRequest.FailMessage, ChatType.System);

                return;
            }

            if (expRateRequest.Decision == Server.Scripting.ScriptHookDecision.Handled)
                return;

            expPoint = (int)(expPoint * expRateRequest.Rate);
        ...
        public void StartGame()
        {
            ...
            // 更细粒度的地图进入事件（区别于 DefaultNPC MapEnter）。
            // 若登录脚本期间发生了跨地图传送（Teleport/MapMovement），对应的 EnterAfter 会由各自路径触发，此处避免重复触发。
            if (CurrentMap == startGameMap)
            {
                TryCSharpPlayerMapEnterAfter(
                    null,
                    Point.Empty,
                    Server.Scripting.PlayerMapTransitionReason.StartGame,
                    redirected: false,
                    Server.Scripting.ScriptHookDecision.Continue);
            }

            TryCSharpPlayerRegionTransition(null, Point.Empty, CurrentMap, CurrentLocation, Server.Scripting.PlayerRegionTransitionReason.StartGame);
        }
        private void StartGameSuccess()
        {
            ...
            // 移动端进入游戏时需要展示公告窗体；为避免“首装/首次进入”因 LastLogoutDate 与 LastUpdate 比较导致不下发，
            // 这里只要公告非空就下发一次，客户端自行决定是否弹出。
            if (!string.IsNullOrWhiteSpace(Settings.Notice.Message) || !string.IsNullOrWhiteSpace(Settings.Notice.Title))
            {
        ...
        public void TownRevive()
        {
            ...
            Map destinationMap = temp;
            Point destinationLocation = bindLocation;
            bool redirected = false;
            var leaveDecision = Server.Scripting.ScriptHookDecision.Continue;

            if (destinationMap != oldMap)
            {
                leaveDecision = TryCSharpPlayerMapLeaveBefore(
                    oldMap,
                    oldLocation,
                    ref destinationMap,
                    ref destinationLocation,
                    Server.Scripting.PlayerMapTransitionReason.TownRevive,
                    out redirected,
                    out _);

                // 复活流程不允许阻止；仅用于重定向目的地。
                if (leaveDecision == Server.Scripting.ScriptHookDecision.Cancel ||
                    leaveDecision == Server.Scripting.ScriptHookDecision.Handled)
                {
                    leaveDecision = Server.Scripting.ScriptHookDecision.Continue;
                }

                if (destinationMap == null || !destinationMap.ValidPoint(destinationLocation))
                {
                    destinationMap = temp;
                    destinationLocation = bindLocation;
                    redirected = false;
                }
            }

            var mapChanged = destinationMap != oldMap;
            ...
            if (mapChanged)
                TryCSharpPlayerMapEnterAfter(oldMap, oldLocation, Server.Scripting.PlayerMapTransitionReason.TownRevive, redirected, leaveDecision);

            TryCSharpPlayerRegionTransition(oldMap, oldLocation, CurrentMap, CurrentLocation, Server.Scripting.PlayerRegionTransitionReason.TownRevive);
        }

        private Server.Scripting.ScriptHookDecision TryCSharpPlayerMapLeaveBefore(
            Map fromMap,
            Point fromLocation,
            ref Map toMap,
            ref Point toLocation,
            Server.Scripting.PlayerMapTransitionReason reason,
            out bool redirected,
            out string failMessage)
        {
            ...
            var scriptsRuntimeActive = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
            if (!scriptsRuntimeActive) return Server.Scripting.ScriptHookDecision.Continue;

            var allowCSharp = Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnPlayerMapLeaveBefore);

            if (!allowCSharp)
            {
                if (Settings.TxtScriptsLogDispatch)
                    MessageQueue.Enqueue($"[Scripts][Dispatch] PlayerMapLeaveBefore {fromMap.Info.FileName}:{Name} -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnPlayerMapLeaveBefore}）");

                return Server.Scripting.ScriptHookDecision.Continue;
            }

            var originalMap = toMap;
            var originalLocation = toLocation;

            var request = new Server.Scripting.PlayerMapLeaveRequest(this, fromMap, fromLocation, toMap, toLocation, reason);
            Envir.CSharpScripts.TryHandlePlayerMapLeaveBefore(this, request);

            var decision = request.Decision;
            failMessage = request.FailMessage ?? string.Empty;

            toMap = request.ToMap;
            toLocation = request.ToLocation;

            redirected = toMap != originalMap || toLocation != originalLocation;

            return decision;
        }

        private void TryCSharpPlayerMapEnterAfter(
            Map fromMap,
            Point fromLocation,
            Server.Scripting.PlayerMapTransitionReason reason,
            bool redirected,
            Server.Scripting.ScriptHookDecision leaveDecision)
        {
            var scriptsRuntimeActive = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
            var allowCSharp = scriptsRuntimeActive && Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnPlayerMapEnterAfter);

            if (!allowCSharp)
            {
                if (scriptsRuntimeActive && Settings.TxtScriptsLogDispatch)
                    MessageQueue.Enqueue($"[Scripts][Dispatch] PlayerMapEnterAfter {CurrentMap?.Info?.FileName}:{Name} -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnPlayerMapEnterAfter}）");

                return;
            }

            var result = new Server.Scripting.PlayerMapEnterResult(this, fromMap, fromLocation, CurrentMap, CurrentLocation, reason, redirected, leaveDecision);
            Envir.CSharpScripts.TryHandlePlayerMapEnterAfter(this, result);
        }

        private void TryCSharpPlayerRegionTransition(
            Map fromMap,
            Point fromLocation,
            Map toMap,
            Point toLocation,
            Server.Scripting.PlayerRegionTransitionReason reason)
        {
            if (toMap == null) return;

            if (fromMap == toMap && fromLocation == toLocation)
            {
                _csharpRegionsLastInitialized = true;
                _csharpRegionsLastMap = toMap;
                _csharpRegionsLastLocation = toLocation;
                return;
            }
            ...
            {
                _csharpActiveRegions.Clear();
                _csharpActiveRegionsScratch.Clear();

                _csharpRegionsLastInitialized = true;
                _csharpRegionsLastMap = toMap;
                _csharpRegionsLastLocation = toLocation;
                _csharpRegionsScriptVersion = -1;
                return;
            }

            var currentVersion = Envir.CSharpScripts.Version;

            if (_csharpRegionsScriptVersion != currentVersion)
            {
                _csharpRegionsScriptVersion = currentVersion;
                _csharpActiveRegions.Clear();
            }

            var allowEnter = Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnPlayerRegionEnter);
            var allowLeave = Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnPlayerRegionLeave);

            var registry = Envir.CSharpScripts.CurrentRegistry;
            var definitions = registry.Regions.GetByMap(toMap.Info.FileName);
            ...
            for (var i = 0; i < definitions.Count; i++)
            {
                ...
                if (def.Contains(toLocation))
                {
                    _csharpActiveRegionsScratch.Add(def.Id);
                }
            }

            if (allowLeave && _csharpActiveRegions.Count > 0)
            {
                foreach (var id in _csharpActiveRegions.ToArray())
                {
                    if (_csharpActiveRegionsScratch.Contains(id)) continue;

                    if (!registry.Regions.TryGet(id, out var def))
                        continue;

                    var e = new Server.Scripting.PlayerRegionEvent(
                        this,
                        fromMap,
                        fromLocation,
                        toMap,
                        toLocation,
                        def,
                        Server.Scripting.PlayerRegionEventType.Leave,
                        reason);

                    Envir.CSharpScripts.TryHandlePlayerRegionLeave(this, e);
                }
            }

            if (allowEnter && _csharpActiveRegionsScratch.Count > 0)
            {
                foreach (var id in _csharpActiveRegionsScratch)
                {
                    if (_csharpActiveRegions.Contains(id)) continue;
                    ...
                    var e = new Server.Scripting.PlayerRegionEvent(
                        ...
                        toLocation,
                        def,
                        Server.Scripting.PlayerRegionEventType.Enter,
                        reason);

                    Envir.CSharpScripts.TryHandlePlayerRegionEnter(this, e);
                }
            }

            _csharpActiveRegions.Clear();
            foreach (var id in _csharpActiveRegionsScratch)
                _csharpActiveRegions.Add(id);
        ...
        private void SyncCSharpActiveRegions(Map map, Point location)
        {
            _csharpActiveRegions.Clear();
            _csharpActiveRegionsScratch.Clear();
            ...
            var registry = Envir.CSharpScripts.CurrentRegistry;
            var definitions = registry.Regions.GetByMap(map.Info.FileName);
            ...
            for (var i = 0; i < definitions.Count; i++)
            {
                ...
                {
                    _csharpActiveRegions.Add(def.Id);
                }
            }
        }
        public override bool Teleport(Map temp, Point location, bool effects = true, byte effectnumber = 0)
        {
            ...
            if (mapChanged)
            {
                leaveDecision = TryCSharpPlayerMapLeaveBefore(
                    ...
                    ref temp,
                    ref location,
                    Server.Scripting.PlayerMapTransitionReason.Teleport,
                    out redirected,
                    out failMessage);
                ...
                if (leaveDecision == Server.Scripting.ScriptHookDecision.Cancel)
                {
                    if (!string.IsNullOrEmpty(failMessage))
                        ReceiveChat(failMessage, ChatType.System);

                    return false;
                }

                if (leaveDecision == Server.Scripting.ScriptHookDecision.Handled)
                {
                    // 脚本选择自行处理传送；此处不再执行引擎默认 Teleport。
                    return CurrentMap != oldMap || CurrentLocation != oldLocation;
                }
            ...
            if (mapChanged)
            {
                TryCSharpPlayerMapEnterAfter(oldMap, oldLocation, Server.Scripting.PlayerMapTransitionReason.Teleport, redirected, leaveDecision);
                TryCSharpPlayerRegionTransition(oldMap, oldLocation, CurrentMap, CurrentLocation, Server.Scripting.PlayerRegionTransitionReason.Teleport);
        ...
        public void Chat(string message, List<ChatItem> linkedItems = null)
        {
            ...
            else if (message.StartsWith("@"))
            {
                ...
                var scriptsRuntimeActive = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
                var allowCSharp = scriptsRuntimeActive && Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnPlayerChatCommand);

                if (allowCSharp)
                {
                    var commandLine = message;
                    var command = parts[0];
                    ...
                    if (Envir.CSharpScripts.TryHandlePlayerChatCommand(this, commandLine, command, args))
                        return;
                }
                else if (scriptsRuntimeActive && Settings.TxtScriptsLogDispatch)
                {
                    MessageQueue.Enqueue($"[Scripts][Dispatch] ChatCommand {parts[0]} -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnPlayerChatCommand}）");
                }
                ...
                switch (parts[0].ToUpper())
                {
                    ...
                    case "BACKUPPLAYER":
                        {
                            ...
                            var backupResult = Envir.SaveArchivedCharacter(info);
                            if (!backupResult.Committed)
                            {
                                ReceiveChat($"玩家 {info.Name} 备份失败：{backupResult.ErrorCode}", ChatType.System);
                                MessageQueue.Enqueue($"玩家 {info.Name} 备份失败：{backupResult.Diagnostics}");
                                return;
                            }
                    ...
                    case "ARCHIVEPLAYER":
                        {
                            ...
                            var archiveResult = Envir.ArchiveCharacter(data);
                            if (!archiveResult.Committed)
                            {
                                ReceiveChat($"玩家 {data.Name} 归档失败：{archiveResult.ErrorCode}", ChatType.System);
                                MessageQueue.Enqueue($"玩家 {data.Name} 归档失败：{archiveResult.Diagnostics}");
                                return;
                            }
                    ...
                    case "LOADPLAYER":
                        {
                            ...
                            var owner = info.AccountInfo;
                            bak.AccountInfo = owner;
                            bak.Mail = info.Mail;
                            bak.GuildIndex = info.GuildIndex;
                            bak.Buffs = info.Buffs;
                            var ownerIndex = owner?.Characters.IndexOf(info) ?? -1;
                            var globalIndex = Envir.CharacterList.IndexOf(info);
                            if (ownerIndex < 0 || globalIndex < 0)
                            {
                                ReceiveChat("角色内存归属不完整，拒绝加载备份", ChatType.System);
                                return;
                            }

                            owner.Characters[ownerIndex] = bak;
                            Envir.CharacterList[globalIndex] = bak;
                            var commit = Envir.SaveAccounts();
                            if (!commit.Committed)
                            {
                                owner.Characters[ownerIndex] = info;
                                Envir.CharacterList[globalIndex] = info;
                                ReceiveChat($"玩家 {info.Name} 回滚失败：{commit.ErrorCode}", ChatType.System);
                                MessageQueue.Enqueue($"玩家 {info.Name} 回滚 checkpoint 失败：{commit.Diagnostics}");
                                return;
                            }

                            ReceiveChat(string.Format("玩家 {0} 已被加载", bak.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format("玩家 {0} 已被 {1} 加载", bak.Name, Name));
                        }
                        break;

                    case "RESTOREPLAYER":
                        {
                            if (!IsGM || parts.Length < 3) return;
                            var account = Envir.GetAccount(parts[2]);
                            if (account == null)
                            {
                                ReceiveChat($"未找到 {parts[2]} 这个账户", ChatType.System);
                                return;
                            }
                            if (account.Characters.Count >= Globals.MaxCharacterCount)
                            {
                                ReceiveChat($"账户 {parts[2]} 已经有 {Globals.MaxCharacterCount} 个角色", ChatType.System);
                                return;
                            }

                            var restoreResult = Envir.RestoreCharacter(parts[1], account);
                            if (!restoreResult.Committed)
                            {
                                ReceiveChat($"玩家 {parts[1]} 恢复失败：{restoreResult.ErrorCode}", ChatType.System);
                                MessageQueue.Enqueue($"玩家 {parts[1]} 恢复失败：{restoreResult.Diagnostics}");
                                return;
                            }

                            ReceiveChat($"玩家 {parts[1]} 已恢复，将在下次安全启动加载时生效", ChatType.System);
                            MessageQueue.Enqueue($"玩家 {parts[1]} 已被 {Name} 恢复到账号 {account.AccountID}");
                        }
                        break;
                    ...
                    case "RELOADSCRIPTS":
                        ...
                        {
                            ReceiveChat("C# 脚本系统未启用（Setup.ini: [CSharpScripts] CSharpScriptsEnabled=false）。", ChatType.System);
                            break;
                        }

                        if (Settings.CSharpScriptsPushModeEnabled)
                        {
                            ReceiveChat("C# 脚本已启用推送模式：已禁用 @ReloadScripts，请使用管理端“脚本调试...”推送并热更。", ChatType.System);
                            break;
                        }

                        if (!Envir.CSharpScripts.Enabled)
                        {
                            ReceiveChat($"C# 脚本系统当前不可用：{Envir.CSharpScripts.LastError}", ChatType.System);
                            break;
                        }

                        var swReloadScripts = System.Diagnostics.Stopwatch.StartNew();
                        Envir.CSharpScripts.Reload();
                        swReloadScripts.Stop();

                        if (!string.IsNullOrWhiteSpace(Envir.CSharpScripts.LastError))
                        {
                            ReceiveChat($"C# 脚本重载失败：{Envir.CSharpScripts.LastError}", ChatType.System);
                        }
                        else
                        {
                            ReceiveChat($"C# 脚本已重载 v{Envir.CSharpScripts.Version}（{swReloadScripts.ElapsedMilliseconds}ms，handlers={Envir.CSharpScripts.LastRegisteredHandlerCount}）", ChatType.Hint);
                        }
                    ...
                    case "QUESTREBIND":
                        ...
                        int? questIndexFilter = null;
                        var recalcProgress = false;
                        var resetTimer = false;

                        var argIndex = 1;
                        if (parts.Length > 1)
                        {
                            if (int.TryParse(parts[1], out var questIndex))
                            {
                                questIndexFilter = questIndex;
                                argIndex = 2;
                            }
                            else if (!string.Equals(parts[1], "RECALC", StringComparison.OrdinalIgnoreCase)
                                && !string.Equals(parts[1], "RESETTIMER", StringComparison.OrdinalIgnoreCase))
                            {
                                targetPlayer = Envir.GetPlayer(parts[1]);
                                if (targetPlayer == null)
                                {
                                    ReceiveChat($"找不到玩家：{parts[1]}", ChatType.System);
                                    break;
                                }

                                argIndex = 2;

                                if (parts.Length > 2 && int.TryParse(parts[2], out questIndex))
                                {
                                    questIndexFilter = questIndex;
                                    argIndex = 3;
                                }
                        ...
                        for (var i = argIndex; i < parts.Length; i++)
                        {
                            if (string.Equals(parts[i], "RECALC", StringComparison.OrdinalIgnoreCase))
                            {
                                recalcProgress = true;
                                continue;
                            }

                            if (string.Equals(parts[i], "RESETTIMER", StringComparison.OrdinalIgnoreCase))
                            {
                                resetTimer = true;
                                continue;
                            }
                        ...
                        if (targetPlayer.CurrentQuests.Count == 0)
                        {
                            ReceiveChat($"{targetPlayer.Name} 当前没有进行中的任务。", ChatType.System);
                            break;
                        }
                        ...
                        for (var i = 0; i < targetPlayer.CurrentQuests.Count; i++)
                        {
                            var quest = targetPlayer.CurrentQuests[i];
                            if (quest == null) continue;

                            if (questIndexFilter != null && quest.Index != questIndexFilter.Value)
                                continue;

                            matched++;

                            if (!quest.TryRebindSnapshot(targetPlayer, recalcProgress, resetTimer, out var rebindError))
                            {
                                failed++;
                                ReceiveChat($"[QuestRebind] Index={quest.Index} 失败：{rebindError}", ChatType.System);
                                continue;
                            }

                            rebound++;
                            targetPlayer.SendUpdateQuest(quest, QuestState.Update);
                        }

                        var scopeText = questIndexFilter != null ? $"Index={questIndexFilter.Value}" : "All";
                        ReceiveChat($"[QuestRebind] Player={targetPlayer.Name} Scope={scopeText} Rebound={rebound}/{matched} Failed={failed} Recalc={(recalcProgress ? "Y" : "N")} ResetTimer={(resetTimer ? "Y" : "N")}", ChatType.System);
                        break;

                    case "SCRIPTSTATUS":
                        if (!IsGM) return;

                        var maxDiagnostics = 30;
                        if (parts.Length > 1)
                        {
                            int.TryParse(parts[1], out maxDiagnostics);
                        }

                        if (maxDiagnostics < 0) maxDiagnostics = 0;
                        if (maxDiagnostics > 200) maxDiagnostics = 200;

                        var scriptsSetupEnabled = Settings.CSharpScriptsEnabled;
                        var scriptsRuntimeEnabled = Envir.CSharpScripts.Enabled;
                        var scriptsVersion = Envir.CSharpScripts.Version;
                        var scriptsHandlers = Envir.CSharpScripts.LastRegisteredHandlerCount;

                        var lastSuccessUtc = Envir.CSharpScripts.LastSuccessUtc;
                        var lastFailureUtc = Envir.CSharpScripts.LastFailureUtc;

                        var lastResult = "无";
                        DateTime? lastResultUtc = null;

                        if (lastFailureUtc != null && (lastSuccessUtc == null || lastFailureUtc > lastSuccessUtc))
                        {
                            lastResult = "失败";
                            lastResultUtc = lastFailureUtc;
                        }
                        else if (lastSuccessUtc != null)
                        {
                            lastResult = "成功";
                            lastResultUtc = lastSuccessUtc;
                        }

                        var lastResultText = lastResultUtc != null
                            ? lastResultUtc.Value.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss")
                            : "N/A";

                        ReceiveChat($"[Scripts] SetupEnabled={scriptsSetupEnabled} Runtime={(scriptsRuntimeEnabled ? "可用" : "不可用")} v{scriptsVersion} handlers={scriptsHandlers} Last={lastResult}@{lastResultText}", ChatType.System);

                        if (!string.IsNullOrWhiteSpace(Envir.CSharpScripts.LastError))
                        {
                            ReceiveChat("[Scripts] LastError: " + Envir.CSharpScripts.LastError, ChatType.System);
                        }

                        var diagnostics = Envir.CSharpScripts.LastDiagnostics;
                        var errorCount = 0;

                        for (var i = 0; i < diagnostics.Count; i++)
                        {
                            if (string.Equals(diagnostics[i].Severity, "Error", StringComparison.OrdinalIgnoreCase))
                                errorCount++;
                        }

                        ReceiveChat($"[Scripts] Diagnostics: total={diagnostics.Count} errors={errorCount} (MaxList={maxDiagnostics})", ChatType.System);

                        if (maxDiagnostics > 0 && diagnostics.Count > 0)
                        {
                            var printed = 0;

                            // 优先打印 Error，便于快速定位。
                            for (var i = 0; i < diagnostics.Count && printed < maxDiagnostics; i++)
                            {
                                if (!string.Equals(diagnostics[i].Severity, "Error", StringComparison.OrdinalIgnoreCase))
                                    continue;

                                ReceiveChat("[Scripts] " + diagnostics[i], ChatType.System);
                                printed++;
                            }

                            // 若没有 Error，则打印前 N 条诊断信息。
                            if (printed == 0)
                            {
                                for (var i = 0; i < diagnostics.Count && printed < maxDiagnostics; i++)
                                {
                            ...
                            if (diagnostics.Count > printed)
                            {
                                ReceiveChat($"[Scripts] ...（已省略 {diagnostics.Count - printed} 条）", ChatType.System);
                            }
                        }
                    ...
                    case "SCRIPTREGISTRY":
                        ...
                        var registry0 = Envir.CSharpScripts.CurrentRegistry;
                        if (registry0 == null)
                        {
                            ReceiveChat("[Scripts] Registry 不可用。", ChatType.System);
                            break;
                        }

                        ReceiveChat($"[Scripts] Registry: handlers={registry0.Count} commands={registry0.CustomCommands.Count} Drops={registry0.Drops.Count} Quests={registry0.Quests.Count} Recipe={registry0.Recipes.Count} Values={registry0.Values.Count} NameLists={registry0.NameLists.Count} Routes={registry0.Routes.Count} TextFiles={registry0.TextFiles.Count} Regions={registry0.Regions.Count}", ChatType.System);
                        break;

                    case "SCRIPTKEY":
                    case "SCRIPTKEYSTATUS":
                        if (!IsGM) return;

                        if (parts.Length < 2)
                        {
                            ReceiveChat("用法：@ScriptKeyStatus <LogicKey>", ChatType.System);
                            ReceiveChat("示例：@ScriptKeyStatus Drops/00Fishing", ChatType.System);
                            ReceiveChat("示例：@ScriptKeyStatus \"Todo+Fixed List\"", ChatType.System);
                            break;
                        }

                        var rawLogicKey = string.Join(" ", parts, 1, parts.Length - 1);
                        if (string.IsNullOrWhiteSpace(rawLogicKey))
                        {
                            ReceiveChat("LogicKey 不能为空。", ChatType.System);
                            break;
                        }

                        var allowCSharp3 = Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(rawLogicKey);
                        var allowFallbackToTxt3 = Server.Scripting.TxtFallbackPolicy.ShouldFallbackToTxt(rawLogicKey);

                        var normalizedKeyText = Server.Scripting.LogicKey.TryNormalize(rawLogicKey, out var normalizedLogicKey) ? normalizedLogicKey : "(无效)";
                        var scriptsRuntimeActive3 = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
                        var scriptsState3 = scriptsRuntimeActive3 ? $"v{Envir.CSharpScripts.Version}, handlers={Envir.CSharpScripts.LastRegisteredHandlerCount}" : $"不可用: {Envir.CSharpScripts.LastError}";

                        ReceiveChat($"[ScriptKey] raw={rawLogicKey} normalized={normalizedKeyText}", ChatType.System);
                        ReceiveChat($"[ScriptKey] （SetupEnabled={Settings.CSharpScriptsEnabled} Runtime={Envir.CSharpScripts.Enabled} C#={scriptsState3}）", ChatType.System);

                        if (!Server.Scripting.LogicKey.TryNormalize(rawLogicKey, out normalizedLogicKey))
                            break;

                        var registry3 = Envir.CSharpScripts.CurrentRegistry;
                        if (registry3 == null)
                        {
                            ReceiveChat("[ScriptKey] Registry 不可用。", ChatType.System);
                            break;
                        }

                        var foundAny = false;

                        if (registry3.Handlers != null && registry3.Handlers.ContainsKey(normalizedLogicKey))
                        {
                            foundAny = true;
                            ReceiveChat("[ScriptKey] Hook: 已注册处理器。", ChatType.System);
                        }

                        if (registry3.Drops != null && registry3.Drops.TryGet(normalizedLogicKey, out var dropTable) && dropTable != null)
                        {
                            foundAny = true;
                            ReceiveChat($"[ScriptKey] Drops: 已注册（entries={dropTable.Drops.Count}）。", ChatType.System);
                        }

                        if (registry3.Quests != null && registry3.Quests.TryGet(normalizedLogicKey, out var questDef) && questDef != null)
                        {
                            foundAny = true;
                            ReceiveChat($"[ScriptKey] Quests: 已注册（desc={questDef.Description.Count} tasks={questDef.KillTasks.Count + questDef.ItemTasks.Count + questDef.CarryItems.Count + questDef.FlagTasks.Count}）。", ChatType.System);
                        }

                        if (registry3.Recipes != null && registry3.Recipes.TryGet(normalizedLogicKey, out var recipeDef) && recipeDef != null)
                        {
                            foundAny = true;
                            ReceiveChat($"[ScriptKey] Recipe: 已注册（tools={recipeDef.Tools.Count} ingredients={recipeDef.Ingredients.Count}）。", ChatType.System);
                        }

                        if (registry3.Values != null && registry3.Values.TryGet(normalizedLogicKey, out var valueDef) && valueDef != null)
                        {
                            foundAny = true;
                            ReceiveChat("[ScriptKey] Values: 已注册。", ChatType.System);
                        }

                        if (registry3.NameLists != null && registry3.NameLists.TryGet(normalizedLogicKey, out var nameListDef) && nameListDef != null)
                        {
                            foundAny = true;
                            ReceiveChat($"[ScriptKey] NameLists: 已注册（values={nameListDef.Values.Count}）。", ChatType.System);
                        }

                        if (registry3.Routes != null && registry3.Routes.TryGet(normalizedLogicKey, out var routeDef) && routeDef != null)
                        {
                            foundAny = true;
                            ReceiveChat($"[ScriptKey] Routes: 已注册（points={routeDef.Route.Count}）。", ChatType.System);
                        }

                        if (registry3.TextFiles != null && registry3.TextFiles.TryGet(normalizedLogicKey, out var textFileDef) && textFileDef != null)
                        {
                            foundAny = true;
                            ReceiveChat($"[ScriptKey] TextFiles: 已注册（lines={textFileDef.Lines.Count}）。", ChatType.System);
                        }

                        if (!foundAny)
                        {
                            ReceiveChat("[ScriptKey] 未找到任何已注册定义/处理器。", ChatType.System);
                        }

                        break;

                    case "SCRIPTTRACESTART":
                        if (!IsGM) return;

                        var maxTraceEntries = 500;
                        if (parts.Length > 1)
                        {
                            int.TryParse(parts[1], out maxTraceEntries);
                        }

                        Server.Scripting.ScriptTrace.Start(this, maxTraceEntries);
                        ReceiveChat($"脚本追踪已开启（最大记录 {maxTraceEntries} 条）。请先触发一次脚本（例如使用物品），再执行 @ScriptTraceStop 导出日志文件。", ChatType.Hint);
                        break;

                    case "SCRIPTTRACESTOP":
                        if (!IsGM) return;

                        if (Server.Scripting.ScriptTrace.StopAndDump(this, out var traceFilePath, out var traceEntryCount, out var traceError))
                        {
                            ReceiveChat($"脚本追踪已导出：{traceEntryCount} 条 -> {traceFilePath}", ChatType.Hint);
                        }
                        else
                        {
                            ReceiveChat($"脚本追踪导出失败：{traceError}", ChatType.System);
                        }

                        break;

                    case "SCRIPTNPCCOVERAGE":
                        if (!IsGM) return;

                        if (parts.Length < 2)
                        {
                            ReceiveChat("用法：@ScriptNpcCoverage <NpcFileName> [StartPageKey] [MaxList]", ChatType.System);
                            ReceiveChat("示例：@ScriptNpcCoverage 沙巴克战旗 [@MAIN] 50", ChatType.System);
                            break;
                        }

                        var npcFileName = parts[1];
                        var startPageKey = parts.Length > 2 ? parts[2] : NPCScript.MainKey;

                        var maxList = 50;
                        if (parts.Length > 3)
                        {
                            int.TryParse(parts[3], out maxList);
                        }

                        if (maxList < 0) maxList = 0;
                        if (maxList > 200) maxList = 200;

                        var scriptsRuntimeActive2 = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
                        var scriptsState2 = scriptsRuntimeActive2 ? $"v{Envir.CSharpScripts.Version}, handlers={Envir.CSharpScripts.LastRegisteredHandlerCount}" : $"不可用: {Envir.CSharpScripts.LastError}";

                        var report = Server.Scripting.NpcScriptCoverage.CheckNpcFile(npcFileName, startPageKey, Envir.CSharpScripts.CurrentRegistry);
                        var hasLegacyNonDialogSections = report.LegacyNonDialogSections.Count > 0;

                        // LoadInfo 是否允许跳过 txt，取决于该 NPC 的 NpcPage KeyPrefix 是否仍允许回落 txt。
                        var fallbackProbeKey = Server.Scripting.ScriptHookKeys.OnNpcPage(report.NpcFileName, "__probe__");
                        var allowFallbackToTxt = Server.Scripting.TxtFallbackPolicy.ShouldFallbackToTxt(fallbackProbeKey);

                        for (var i = 0; i < report.Diagnostics.Count; i++)
                        {
                            ReceiveChat($"[NpcCoverage] {report.Diagnostics[i]}", ChatType.System);
                        }

                        var skipListConfigured = !string.IsNullOrWhiteSpace(Settings.CSharpScriptsSkipTxtNpcLoadNpcFiles);
                        var skipListHit = !skipListConfigured || Server.Scripting.NpcTxtSkipPolicy.IsNpcFileInAllowlist(npcFileName);
                        var skipListText = skipListConfigured ? (skipListHit ? "命中" : "未命中") : "空(全量)";

                        ReceiveChat($"[NpcCoverage] {report.NpcFileName} start={report.StartPageKey} reachable={report.ReachablePageKeys.Count} missing={report.MissingCSharpPageKeys.Count} policySkip={report.PolicyDisallowedPageKeys.Count} legacySections={report.LegacyNonDialogSections.Count}（ NoTxtFallbackEnabled={Settings.CSharpScriptsNoTxtFallbackEnabled}, AllowFallbackToTxt={allowFallbackToTxt}, SkipTxtNpcLoad={Settings.CSharpScriptsSkipTxtNpcLoad}, SkipList={skipListText}, C#={scriptsState2}）", ChatType.System);

                        if (hasLegacyNonDialogSections)
                        {
                            ReceiveChat($"[NpcCoverage] 注意：该 NPC txt 包含非对话 section：{string.Join(", ", report.LegacyNonDialogSections)}。启用 SkipTxtNpcLoad 将跳过这些内容加载。", ChatType.System);
                        }

                        var canNoTxt = scriptsRuntimeActive2 && !allowFallbackToTxt && report.IsCoverageComplete && !hasLegacyNonDialogSections;

                        if (canNoTxt)
                        {
                            ReceiveChat("[NpcCoverage] 判定：可对该 NPC 开启“无 txt 运行”。", ChatType.Hint);

                            if (!Settings.CSharpScriptsSkipTxtNpcLoad)
                            {
                                ReceiveChat("[NpcCoverage] 提示：要真正跳过加载该 NPC 的 txt，请在 Setup.ini 设置 CSharpScriptsSkipTxtNpcLoad=True，并（推荐）配置 CSharpScriptsSkipTxtNpcLoadNpcFiles=该NPC文件名（支持 ;/, 分隔，可含子目录）。", ChatType.Hint);
                            }
                            else if (skipListConfigured && !skipListHit)
                            {
                                ReceiveChat("[NpcCoverage] 提示：已开启 SkipTxtNpcLoad，但该 NPC 不在 SkipTxtNpcLoadNpcFiles 白名单中；请将其加入白名单，否则仍会加载 txt。", ChatType.Hint);
                            }
                            else if (!skipListConfigured)
                            {
                                ReceiveChat("[NpcCoverage] 注意：SkipTxtNpcLoadNpcFiles 为空时表示“全量跳过所有 Normal NPC 的 txt”。建议改用白名单，避免误伤未迁移的 NPC。", ChatType.System);
                            }
                        }
                        else
                        {
                            if (!scriptsRuntimeActive2)
                                ReceiveChat("[NpcCoverage] 原因：C# 脚本系统未启用或不可用。", ChatType.System);

                            if (allowFallbackToTxt)
                            {
                                var npcPrefixHint = $"NPCs/{report.NpcFileName}";
                                ReceiveChat($"[NpcCoverage] 原因：仍允许回落TXT（无 txt 运行需要对该 NPC 禁止回落；可关闭 FallbackToTxt，或在 NoTxtFallbackKeyPrefixes 中加入 {npcPrefixHint}）。", ChatType.System);
                            }

                            if (hasLegacyNonDialogSections)
                                ReceiveChat("[NpcCoverage] 原因：该 NPC txt 仍包含非对话 section（SkipTxtNpcLoad 会导致这些内容不再加载）。", ChatType.System);

                            if (report.PolicyDisallowedPageKeys.Count > 0)
                                ReceiveChat("[NpcCoverage] 原因：ScriptDispatchPolicy 禁用了部分页的 C# 分发（DisabledKeys/PreferKeyPrefixes）。", ChatType.System);

                            if (report.MissingCSharpPageKeys.Count > 0)
                                ReceiveChat("[NpcCoverage] 原因：存在可达页面缺少 C# 处理器。", ChatType.System);
                        }

                        if (maxList > 0 && report.MissingCSharpPageKeys.Count > 0)
                        {
                            ReceiveChat($"[NpcCoverage] 缺少 C# Handler 的页面（前 {Math.Min(maxList, report.MissingCSharpPageKeys.Count)} 个）：", ChatType.System);
                            for (var i = 0; i < report.MissingCSharpPageKeys.Count && i < maxList; i++)
                            {
                                ReceiveChat("  " + report.MissingCSharpPageKeys[i], ChatType.System);
                            }
                        }

                        if (maxList > 0 && report.PolicyDisallowedPageKeys.Count > 0)
                        {
                            ReceiveChat($"[NpcCoverage] 被 ScriptDispatchPolicy 禁用的页面（前 {Math.Min(maxList, report.PolicyDisallowedPageKeys.Count)} 个）：", ChatType.System);
                            for (var i = 0; i < report.PolicyDisallowedPageKeys.Count && i < maxList; i++)
                            {
                                ReceiveChat("  " + report.PolicyDisallowedPageKeys[i], ChatType.System);
                            }
                        }

                        break;

                    case "SCRIPTNPCCOVERAGEDUMP":
                        if (!IsGM) return;

                        if (parts.Length < 2)
                        {
                            ReceiveChat("用法：@ScriptNpcCoverageDump <NpcFileName> [StartPageKey]", ChatType.System);
                            ReceiveChat("示例：@ScriptNpcCoverageDump 沙巴克战旗 [@MAIN]", ChatType.System);
                            break;
                        }

                        var npcFileNameDump = parts[1];
                        var startPageKeyDump = parts.Length > 2 ? parts[2] : NPCScript.MainKey;

                        var reportDump = Server.Scripting.NpcScriptCoverage.CheckNpcFile(npcFileNameDump, startPageKeyDump, Envir.CSharpScripts.CurrentRegistry);
                        if (Server.Scripting.NpcScriptCoverageDumper.DumpNpcLatest(reportDump, out var npcCoverageJsonPath, out var npcCoverageMdPath, out var npcCoverageDumpError))
                        {
                            ReceiveChat($"[NpcCoverageDump] 已导出：{npcCoverageJsonPath}", ChatType.Hint);
                            ReceiveChat($"[NpcCoverageDump] 已导出：{npcCoverageMdPath}", ChatType.Hint);
                        }
                        else
                        {
                            ReceiveChat($"[NpcCoverageDump] 导出失败：{npcCoverageDumpError}", ChatType.System);
                        }

                        break;

                    case "SCRIPTNPCCOVERAGEALLDUMP":
                        if (!IsGM) return;

                        var startPageKeyAllDump = parts.Length > 1 ? parts[1] : NPCScript.MainKey;

                        if (Server.Scripting.NpcScriptCoverageDumper.DumpAllNpcLatest(startPageKeyAllDump, Envir.CSharpScripts.CurrentRegistry, out var allNpcCoverageJsonPath, out var allNpcCoverageMdPath, out var allNpcCoverageDumpError))
                        {
                            ReceiveChat($"[NpcCoverageAllDump] 已导出：{allNpcCoverageJsonPath}", ChatType.Hint);
                            ReceiveChat($"[NpcCoverageAllDump] 已导出：{allNpcCoverageMdPath}", ChatType.Hint);
                        }
                        else
                        {
                            ReceiveChat($"[NpcCoverageAllDump] 导出失败：{allNpcCoverageDumpError}", ChatType.System);
                        }

                        break;

                    case "SCRIPTNPCCOVERAGEALL":
                        if (!IsGM) return;

                        var startPageKeyAll = parts.Length > 1 ? parts[1] : NPCScript.MainKey;

                        var maxListAll = 50;
                        if (parts.Length > 2)
                        {
                            int.TryParse(parts[2], out maxListAll);
                        }

                        if (maxListAll < 0) maxListAll = 0;
                        if (maxListAll > 200) maxListAll = 200;

                        var scriptsRuntimeActiveCoverageAll = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
                        var scriptsStateCoverageAll = scriptsRuntimeActiveCoverageAll ? $"v{Envir.CSharpScripts.Version}, handlers={Envir.CSharpScripts.LastRegisteredHandlerCount}" : $"不可用: {Envir.CSharpScripts.LastError}";

                        var npcFilesAll = Server.Scripting.NpcScriptCoverage.ListNpcFileNames(out var npcListError);
                        if (!string.IsNullOrWhiteSpace(npcListError))
                        {
                            ReceiveChat($"[NpcCoverageAll] 枚举 NPC 文件失败：{npcListError}", ChatType.System);
                            break;
                        }

                        if (npcFilesAll.Count == 0)
                        {
                            ReceiveChat("[NpcCoverageAll] 未找到任何 NPC 脚本文件。", ChatType.System);
                            break;
                        }

                        var registryAll = Envir.CSharpScripts.CurrentRegistry;

                        var candidatesAll = new List<string>();
                        var legacySectionsAll = 0;
                        var missingAll = 0;
                        var policySkipAll = 0;
                        var allowFallbackAll = 0;

                        for (var i = 0; i < npcFilesAll.Count; i++)
                        {
                            var npcFile = npcFilesAll[i];

                            var reportAll = Server.Scripting.NpcScriptCoverage.CheckNpcFile(npcFile, startPageKeyAll, registryAll);
                            var hasLegacyNonDialogSectionsAll = reportAll.LegacyNonDialogSections.Count > 0;

                            if (hasLegacyNonDialogSectionsAll) legacySectionsAll++;
                            if (reportAll.MissingCSharpPageKeys.Count > 0) missingAll++;
                            if (reportAll.PolicyDisallowedPageKeys.Count > 0) policySkipAll++;

                            var fallbackProbeKeyAll = Server.Scripting.ScriptHookKeys.OnNpcPage(reportAll.NpcFileName, "__probe__");
                            var allowFallbackToTxtAll = Server.Scripting.TxtFallbackPolicy.ShouldFallbackToTxt(fallbackProbeKeyAll);
                            if (allowFallbackToTxtAll) allowFallbackAll++;

                            var canNoTxtAll = scriptsRuntimeActiveCoverageAll && !allowFallbackToTxtAll && reportAll.IsCoverageComplete && !hasLegacyNonDialogSectionsAll;
                            if (canNoTxtAll)
                            {
                                candidatesAll.Add(reportAll.NpcFileName);
                            }
                        }

                        var skipListConfiguredAll = !string.IsNullOrWhiteSpace(Settings.CSharpScriptsSkipTxtNpcLoadNpcFiles);
                        var skipListHitCountAll = 0;

                        if (skipListConfiguredAll)
                        {
                            for (var i = 0; i < candidatesAll.Count; i++)
                            {
                                if (Server.Scripting.NpcTxtSkipPolicy.IsNpcFileInAllowlist(candidatesAll[i]))
                                    skipListHitCountAll++;
                            }
                        }

                        var dirAll = Path.Combine(".", "Logs", "NpcCoverage");
                        var candidatesFileAll = Path.Combine(dirAll, "npc-no-txt-candidates-latest.txt");
                        var wroteCandidatesAll = false;

                        try
                        {
                            Directory.CreateDirectory(dirAll);

                            var tempFile = candidatesFileAll + ".tmp";
                            File.WriteAllLines(tempFile, candidatesAll, new System.Text.UTF8Encoding(encoderShouldEmitUTF8Identifier: false));
                            File.Move(tempFile, candidatesFileAll, overwrite: true);

                            wroteCandidatesAll = true;
                        }
                        catch (Exception ex)
                        {
                            ReceiveChat($"[NpcCoverageAll] 候选清单导出失败：{ex.Message}", ChatType.System);
                        }

                        ReceiveChat($"[NpcCoverageAll] files={npcFilesAll.Count} candidates={candidatesAll.Count} legacySections={legacySectionsAll} missing={missingAll} policySkip={policySkipAll} （SkipTxtNpcLoad={Settings.CSharpScriptsSkipTxtNpcLoad}, SkipListConfigured={skipListConfiguredAll}, SkipListHit={skipListHitCountAll}, C#={scriptsStateCoverageAll}）", ChatType.System);

                        if (wroteCandidatesAll)
                        {
                            ReceiveChat($"[NpcCoverageAll] 候选清单已导出：{candidatesFileAll}", ChatType.Hint);
                        }

                        if (maxListAll > 0 && candidatesAll.Count > 0)
                        {
                            ReceiveChat($"[NpcCoverageAll] 候选 NPC（前 {Math.Min(maxListAll, candidatesAll.Count)} 个）：", ChatType.System);
                            for (var i = 0; i < candidatesAll.Count && i < maxListAll; i++)
                            {
                                ReceiveChat("  " + candidatesAll[i], ChatType.System);
                            }
                        }
                        else if (candidatesAll.Count == 0)
                        {
                            ReceiveChat("[NpcCoverageAll] 没有找到可无 txt 运行的 NPC（请确认 C# 脚本系统启用且对应页已迁移，且对该 NPC 禁止回落 TXT）。", ChatType.System);
                        }

                        break;

                    case "TXTINSERTINCLUDEDUMP":
                        if (!IsGM) return;

                        var maxListInsertInclude = 50;
                        if (parts.Length > 1)
                        {
                            int.TryParse(parts[1], out maxListInsertInclude);
                        }

                        if (maxListInsertInclude < 0) maxListInsertInclude = 0;
                        if (maxListInsertInclude > 200) maxListInsertInclude = 200;

                        if (!Server.Scripting.TxtInsertIncludeScanner.TryScanEnvir(out var insertIncludeEntries, out var insertIncludeScanError))
                        {
                            ReceiveChat($"[TxtScripts] #INSERT/#INCLUDE 扫描失败：{insertIncludeScanError}", ChatType.System);
                            break;
                        }
                        ...
                        for (var i = 0; i < insertIncludeEntries.Count; i++)
                        {
                            totalInsert += insertIncludeEntries[i].InsertCount;
                            totalInclude += insertIncludeEntries[i].IncludeCount;
                        }

                        if (Server.Scripting.TxtInsertIncludeScanner.DumpLatest(insertIncludeEntries, out var insertIncludeFilePath, out var insertIncludeDumpError))
                        {
                            ReceiveChat($"[TxtScripts] #INSERT/#INCLUDE 扫描完成：files={insertIncludeEntries.Count} insert={totalInsert} include={totalInclude}（EnableInsertInclude={Settings.TxtScriptsEnableInsertInclude}）", ChatType.System);
                            ReceiveChat($"[TxtScripts] 已导出：{insertIncludeFilePath}", ChatType.Hint);
                        }
                        else
                        {
                            ReceiveChat($"[TxtScripts] #INSERT/#INCLUDE 扫描完成：files={insertIncludeEntries.Count} insert={totalInsert} include={totalInclude}（EnableInsertInclude={Settings.TxtScriptsEnableInsertInclude}）", ChatType.System);
                            ReceiveChat($"[TxtScripts] 导出失败：{insertIncludeDumpError}", ChatType.System);
                        }

                        if (maxListInsertInclude > 0 && insertIncludeEntries.Count > 0)
                        {
                            ReceiveChat($"[TxtScripts] 包含 #INSERT/#INCLUDE 的文件（前 {Math.Min(maxListInsertInclude, insertIncludeEntries.Count)} 个）：", ChatType.System);
                            for (var i = 0; i < insertIncludeEntries.Count && i < maxListInsertInclude; i++)
                            {
                                var e = insertIncludeEntries[i];
                                ReceiveChat($"  {e.RelativePath} (insert={e.InsertCount}, include={e.IncludeCount})", ChatType.System);
                            }
                        }
                    ...
                    case "TXTUSAGEDUMP":
                        ...
                        {
                            ReceiveChat("未启用 TxtScriptsUsageTraceEnabled（Setup.ini: [TxtScripts] TxtScriptsUsageTraceEnabled=True）。当前不会记录运行时引用；导出的统计将为空或不完整。", ChatType.System);
                        }

                        if (Server.Scripting.TxtUsageTracker.DumpLatest(out var txtUsageFilePath, out var txtUsageError))
                        {
                            ReceiveChat($"TxtUsage 已导出：{txtUsageFilePath}", ChatType.Hint);
                        }
                        else
                        {
                            ReceiveChat($"TxtUsage 导出失败：{txtUsageError}", ChatType.System);
                        }
                    ...
                    case "TXTUSAGECLEAR":
                        ...
                        Server.Scripting.TxtUsageTracker.Clear();
                        ReceiveChat("TxtUsage 已清空。", ChatType.Hint);
                        break;

                    case "SCRIPTMETRICSDUMP":
                        if (!IsGM) return;

                        if (!Settings.ScriptsRuntimeMetricsEnabled)
                        {
                            ReceiveChat("未启用 ScriptsRuntimeMetricsEnabled（Setup.ini: [ScriptMetrics] Enabled=True）。当前仍可导出，但数据可能为空或不完整。", ChatType.System);
                        }

                        if (Server.Scripting.ScriptRuntimeMetrics.DumpLatest(out var metricsFilePath, out var metricsEntryCount, out var metricsDumpError))
                        {
                            ReceiveChat($"ScriptMetrics 已导出：entries={metricsEntryCount}", ChatType.System);
                            ReceiveChat($"ScriptMetrics 文件：{metricsFilePath}", ChatType.Hint);
                        }
                        else
                        {
                            ReceiveChat($"ScriptMetrics 导出失败：{metricsDumpError}", ChatType.System);
                        }

                        break;

                    case "SCRIPTMETRICSCLEAR":
                        if (!IsGM) return;

                        Server.Scripting.ScriptRuntimeMetrics.Clear();
                        ReceiveChat("ScriptMetrics 已清空。", ChatType.Hint);
                        break;

                    case "SCRIPTHOOKSDUMP":
                        if (!IsGM) return;
                        ...
                        try
                        {
                            ...
                            var filePath = Path.Combine(dir, "script-hooks-latest.txt");
                            var tempFilePath = filePath + ".tmp";

                            var keys = registry != null && registry.Handlers != null
                                ? registry.Handlers.Keys.ToList()
                                : new List<string>();
                            keys.Sort(StringComparer.Ordinal);

                            var useItemShapePrefix = Server.Scripting.ScriptHookKeys.OnPlayerUseItem + "/";
                            var useItemShapeCount = 0;

                            for (var i = 0; i < keys.Count; i++)
                            {
                                if (keys[i].StartsWith(useItemShapePrefix, StringComparison.Ordinal))
                                    useItemShapeCount++;
                            }

                            var customCommands = registry != null && registry.CustomCommands != null
                                ? registry.CustomCommands.ToList()
                                : new List<string>();
                            customCommands.Sort(StringComparer.OrdinalIgnoreCase);

                            var lines = new List<string>(keys.Count + 64)
                            {
                                $"GeneratedAtUtc: {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}",
                                $"Scripts: EnabledSetting={Settings.CSharpScriptsEnabled}, RuntimeEnabled={Envir.CSharpScripts.Enabled}, Version={Envir.CSharpScripts.Version}, Handlers={(registry?.Handlers?.Count ?? 0)}",
                                "",
                                "[SystemHooks]",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerLogin}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerLogin) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerLevelUp}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerLevelUp) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerDie}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerDie) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerMapEnter}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerMapEnter) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerMapCoord}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerMapCoord) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerTrigger}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerTrigger) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerCustomCommand}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerCustomCommand) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerAcceptQuest}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerAcceptQuest) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerFinishQuest}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerFinishQuest) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerDaily}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerDaily) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnClientEvent}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnClientEvent) == true ? "Y" : "")}",
                                $"- {Server.Scripting.ScriptHookKeys.OnPlayerUseItem}: {(registry?.Handlers?.ContainsKey(Server.Scripting.ScriptHookKeys.OnPlayerUseItem) == true ? "Y" : "")} (shapes={useItemShapeCount})",
                                "",
                                "[CustomCommands]",
                            };
                            ...
                            else
                            {
                                ...
                                {
                                    lines.Add("- " + customCommands[i]);
                                }
                            }

                            lines.Add("");
                            lines.Add("[AllHandlers]");
                            for (var i = 0; i < keys.Count; i++)
                            {
                                lines.Add(keys[i]);
                            }

                            File.WriteAllLines(tempFilePath, lines, new System.Text.UTF8Encoding(encoderShouldEmitUTF8Identifier: false));
                            File.Move(tempFilePath, filePath, overwrite: true);

                            ReceiveChat($"ScriptHooks 已导出：handlers={registry?.Handlers?.Count ?? 0} customCommands={customCommands.Count}", ChatType.System);
                            ReceiveChat($"文件：{filePath}", ChatType.Hint);
                        }
                        catch (Exception ex)
                        {
                            ReceiveChat($"ScriptHooks 导出失败：{ex.Message}", ChatType.System);
                        }
                ...
                bool matchedTxtCustomCommand = false;
                foreach (string command in Envir.CustomCommands)
                {
                    if (string.Compare(parts[0], command, true) != 0) continue;

                    matchedTxtCustomCommand = true;
                    CallDefaultNPC(DefaultNPCType.CustomCommand, parts[0]);
                }

                if (!matchedTxtCustomCommand && Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled)
                {
                    if (Envir.CSharpScripts.CurrentRegistry.IsCustomCommand(parts[0]))
                    {
                        CallDefaultNPC(DefaultNPCType.CustomCommand, parts[0]);
                    }
                }
        ...
        public override bool CheckMovement(Point location)
        {
            ...
            // C# 脚本：懒加载“坐标触发点”（用于 DefaultNPCType.MapCoord）。
            // 说明：legacy 会在 DefaultNPC 脚本 LoadInfo 扫描 [@_MAPCOORD(...)] 并写入 ActiveCoords；
            // 在迁移/跳过加载 txt 时，需要由 C# 脚本提供并写入 ActiveCoords。
            Envir.EnsureCSharpActiveMapCoordsApplied();
            ...
            for (int i = 0; i < CurrentMap.Info.Movements.Count; i++)
            {
                ...
                if (mapChanged)
                {
                    leaveDecision = TryCSharpPlayerMapLeaveBefore(
                        ...
                        ref temp,
                        ref destination,
                        Server.Scripting.PlayerMapTransitionReason.MapMovement,
                        out redirected,
                        out failMessage);
                    ...
                    if (leaveDecision == Server.Scripting.ScriptHookDecision.Handled)
                    {
                        // 脚本选择自行处理换图；认为本次 MapMovement 已被消费。
                        return CurrentMap != oldMap || CurrentLocation != oldLocation;
                    }

                    if (temp == null || !temp.ValidPoint(destination))
                    {
                        if (!string.IsNullOrEmpty(failMessage))
                            ReceiveChat(failMessage, ChatType.System);
                ...
                CompleteMapMovement(temp, destination, CurrentMap, CurrentLocation, Server.Scripting.PlayerMapTransitionReason.MapMovement, redirected, leaveDecision);
                return true;
            }
        ...
        private void CompleteMapMovement(params object[] data)
        {
            ...
            var reason = data.Length > 4 && data[4] is Server.Scripting.PlayerMapTransitionReason r
                ? r
                : Server.Scripting.PlayerMapTransitionReason.Unknown;
            var redirected = data.Length > 5 && data[5] is bool b && b;
            var leaveDecision = data.Length > 6 && data[6] is Server.Scripting.ScriptHookDecision d
                ? d
                : Server.Scripting.ScriptHookDecision.Continue;
            ...
            if (mapChanged)
            {
                TryCSharpPlayerMapEnterAfter(checkmap, checklocation, reason, redirected, leaveDecision);
                TryCSharpPlayerRegionTransition(checkmap, checklocation, CurrentMap, CurrentLocation, Server.Scripting.PlayerRegionTransitionReason.MapMovement);
                CallDefaultNPC(DefaultNPCType.MapEnter, CurrentMap.Info.FileName);
                GroupMemberMapNameChanged();
            }
            else
        ...
        public override void UseItem(ulong id)
        {
            ...
            var scriptsRuntimeActive = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
            var allowCSharpCheck = scriptsRuntimeActive && Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnPlayerItemUseCheck);

            if (!allowCSharpCheck && scriptsRuntimeActive && Settings.TxtScriptsLogDispatch)
            {
                MessageQueue.Enqueue($"[Scripts][Dispatch] UseItem -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnPlayerItemUseCheck}）");
            }
            ...
            if (allowCSharpCheck && item != null && index != -1)
            {
                var req = new Server.Scripting.PlayerItemUseCheckRequest(MirGridType.Inventory, item, index, Dead);
                Envir.CSharpScripts.TryHandlePlayerItemUseCheck(this, req);

                if (req.Decision == Server.Scripting.ScriptHookDecision.Cancel)
                {
                    if (!string.IsNullOrEmpty(req.FailMessage))
                        ReceiveChat(req.FailMessage, ChatType.System);
        ...
        public void PickUp()
        {
            ...
            if (!allowCSharpCheck && scriptsRuntimeActive && Settings.TxtScriptsLogDispatch)
            {
                MessageQueue.Enqueue($"[Scripts][Dispatch] PickUp -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnPlayerItemPickupCheck}）");
            }
            ...
            for (int i = 0; i < cell.Objects.Count; i++)
            {
                ...
                var legacyHasPickupPermission = ob.Owner == null || ob.Owner == this || IsGroupMember(ob.Owner);
                var legacyCanGain = item.Item != null ? CanGainItem(item.Item) : CanGainGold(item.Gold);

                if (allowCSharpCheck)
                {
                    var req = new Server.Scripting.PlayerItemPickupCheckRequest(
                        Server.Scripting.PlayerItemPickupSource.Player,
                        this,
                        item,
                        legacyHasPickupPermission,
                        legacyCanGain);

                    Envir.CSharpScripts.TryHandlePlayerItemPickupCheck(this, req);

                    if (req.Decision == Server.Scripting.ScriptHookDecision.Cancel)
                    {
                        if (!string.IsNullOrEmpty(req.FailMessage) && string.IsNullOrEmpty(scriptFailMessage))
                            scriptFailMessage = req.FailMessage;

                        continue;
                    }

                    if (req.IgnoreOwnership)
                        legacyHasPickupPermission = true;
                }

                if (!legacyHasPickupPermission) //Or Group member.
                {
                    sendFail = true;
                    continue;
                }
            ...
            if (!string.IsNullOrEmpty(scriptFailMessage))
                ReceiveChat(scriptFailMessage, ChatType.System);
            else if (sendFail)
        ...
        public void CallDefaultNPC(DefaultNPCType type, params object[] value)
        {
            ...
            if (scriptsRuntimeActive)
            {
                ...
                switch (type)
                {
                    case DefaultNPCType.Login:
                        policyKey = "SystemScripts/00Default/Login";
                        break;
                    case DefaultNPCType.LevelUp:
                        policyKey = "SystemScripts/00Default/LevelUp";
                        break;
                    case DefaultNPCType.Die:
                        policyKey = "SystemScripts/00Default/Die";
                        break;
                    case DefaultNPCType.UseItem:
                        policyKey = "SystemScripts/00Default/UseItems";
                        break;
                    case DefaultNPCType.MapEnter:
                        policyKey = "SystemScripts/00Default/MapEnter";
                        break;
                    case DefaultNPCType.MapCoord:
                        policyKey = "SystemScripts/00Default/MapCoords";
                        break;
                    case DefaultNPCType.Trigger:
                        policyKey = "SystemScripts/00Default/Trigger";
                        break;
                    case DefaultNPCType.CustomCommand:
                        policyKey = "SystemScripts/00Default/CustomCommands";
                        break;
                    case DefaultNPCType.OnAcceptQuest:
                        policyKey = "SystemScripts/00Default/OnAcceptQuests";
                        break;
                    case DefaultNPCType.OnFinishQuest:
                        policyKey = "SystemScripts/00Default/OnFinishQuests";
                        break;
                    case DefaultNPCType.Daily:
                        policyKey = "SystemScripts/00Default/Daily";
                        break;
                    case DefaultNPCType.Client:
                        policyKey = "SystemScripts/00Default/Client";
                        break;
                }
                ...
                if (!allowCSharp)
                {
                    ...
                    if (Settings.TxtScriptsLogDispatch)
                        MessageQueue.Enqueue($"[Scripts][Dispatch] DefaultNPC {type} -> 跳过C#（policy key={policyKey}）");
                }
                else
                {
                switch (type)
                {
                    ...
                    case DefaultNPCType.Die:
                        handled = Envir.CSharpScripts.TryHandlePlayerDie(this);
                        break;

                    case DefaultNPCType.UseItem:
                        if (value == null || value.Length < 1) return;
                        handled = Envir.CSharpScripts.TryHandlePlayerUseItem(this, Convert.ToInt32(value[0]));
                        break;

                    case DefaultNPCType.MapEnter:
                        if (value == null || value.Length < 1) return;
                        handled = Envir.CSharpScripts.TryHandlePlayerMapEnter(this, value[0]?.ToString() ?? string.Empty);
                        break;

                    case DefaultNPCType.MapCoord:
                        if (value == null || value.Length < 3) return;
                        handled = Envir.CSharpScripts.TryHandlePlayerMapCoord(
                            this,
                            value[0]?.ToString() ?? string.Empty,
                    ...
                    case DefaultNPCType.Trigger:
                        if (value == null || value.Length < 1) return;
                        handled = Envir.CSharpScripts.TryHandlePlayerTrigger(this, value[0]?.ToString() ?? string.Empty);
                        break;

                    case DefaultNPCType.CustomCommand:
                        if (value == null || value.Length < 1) return;
                        handled = Envir.CSharpScripts.TryHandlePlayerCustomCommand(this, value[0]?.ToString() ?? string.Empty);
                        break;

                    case DefaultNPCType.OnAcceptQuest:
                        if (value == null || value.Length < 1) return;
                        handled = Envir.CSharpScripts.TryHandlePlayerAcceptQuest(this, Convert.ToInt32(value[0]));
                        break;

                    case DefaultNPCType.OnFinishQuest:
                        if (value == null || value.Length < 1) return;
                        handled = Envir.CSharpScripts.TryHandlePlayerFinishQuest(this, Convert.ToInt32(value[0]));
                        break;

                    case DefaultNPCType.Daily:
                        handled = Envir.CSharpScripts.TryHandlePlayerDaily(this);
                        break;

                    case DefaultNPCType.Client:
                        handled = Envir.CSharpScripts.TryHandleClientEvent(this, value != null && value.Length > 0 ? value[0] : null);
                        break;
                ...
                {
                    if (Settings.TxtScriptsLogDispatch)
                        MessageQueue.Enqueue($"[Scripts][Dispatch] DefaultNPC {type} -> C#（v{Envir.CSharpScripts.Version}，handlers={Envir.CSharpScripts.LastRegisteredHandlerCount}）");
                    return;
                }
            ...
            if (Settings.TxtScriptsLogDispatch && Settings.CSharpScriptsEnabled)
            {
        ...
        public void SellItem(ulong uniqueID, ushort count)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                var previewItem = temp;
                if (temp.Info.StackSize > 1 && count != temp.Count)
                {
                    previewItem = temp.Clone();
                    previewItem.Count = count;
                }

                var priceRequest = script.GetSellPriceRequest(this, previewItem, count);
                ...
                if (priceRequest.Decision == Server.Scripting.ScriptHookDecision.Cancel)
                {
                    if (!string.IsNullOrEmpty(priceRequest.FailMessage))
                        ReceiveChat(priceRequest.FailMessage, ChatType.System);

                    Enqueue(p);
                    return;
                }

                if (priceRequest.Decision == Server.Scripting.ScriptHookDecision.Handled)
                {
                    Enqueue(p);
                    return;
                }

                if ((ulong)priceRequest.TotalPrice + Account.Gold > uint.MaxValue)
                {
                    Enqueue(p);
                    return;
                }
                ...
                p.Success = true;
                Enqueue(p);
                GainGold(priceRequest.TotalPrice);

                if (ob.Conq != null && priceRequest.DepositTaxToConquest && priceRequest.TaxAmount > 0)
                {
                    ob.Conq.GuildInfo.GoldStorage += priceRequest.TaxAmount;
                }
        ...
        #region Consignment
        private Server.Scripting.MarketFeeRequest ResolveMarketFee(
            NPCObject npc,
            Server.Scripting.MarketFeeOperation operation,
            ...
            uint price)
        {
            ...
            uint baseListingFee = 0;
            uint listingFee = 0;

            switch (operation)
            {
                case Server.Scripting.MarketFeeOperation.Listing:
                    baseListingFee = panelType == MarketPanelType.Consign ? Globals.ConsignmentCost : Globals.AuctionCost;
                    listingFee = baseListingFee;
                    break;
                case Server.Scripting.MarketFeeOperation.Commission:
                    baseListingFee = 0;
                    listingFee = 0;
                    break;
                default:
            ...
            var baseCommissionRate = Globals.Commission;
            var commissionRate = baseCommissionRate;

            var request = new Server.Scripting.MarketFeeRequest(
                this,
                npc,
                operation,
                panelType,
            ...
            var allowBefore = scriptsRuntimeActive &&
                              Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMarketFeeBefore) &&
                              Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMarketFeeBeforeOperation(operation));
            var allowAfter = scriptsRuntimeActive &&
                             Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMarketFeeAfter) &&
                             Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMarketFeeAfterOperation(operation));

            if (allowBefore)
            {
                Envir.CSharpScripts.TryHandleMarketFeeBefore(request);
            }
            ...
            if (allowAfter)
            {
                var result = new Server.Scripting.MarketFeeResult(
                    this,
                    npc,
                    operation,
                    panelType,
                    ...
                    item,
                    price,
                    request.BaseListingFee,
                    request.ListingFee,
                    request.BaseCommissionRate,
                    request.CommissionRate,
                    request.Decision == Server.Scripting.ScriptHookDecision.Continue,
                    request.Decision);

                Envir.CSharpScripts.TryHandleMarketFeeAfter(result);
            }
        ...
        public void ConsignItem(ulong uniqueID, uint price, MarketPanelType panelType)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                var feeRequest = ResolveMarketFee(ob, Server.Scripting.MarketFeeOperation.Listing, panelType, type, null, temp, price);

                if (feeRequest.Decision == Server.Scripting.ScriptHookDecision.Cancel)
                {
                    if (!string.IsNullOrEmpty(feeRequest.FailMessage))
                        ReceiveChat(feeRequest.FailMessage, ChatType.System);

                    Enqueue(p);
                    return;
                }

                if (feeRequest.Decision == Server.Scripting.ScriptHookDecision.Handled)
                    return;

                uint cost = feeRequest.ListingFee;
        ...
        public void MarketSellNow(ulong auctionID)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                foreach (AuctionInfo auction in Account.Auctions)
                {
                    ...
                    var feeRequest = ResolveMarketFee(ob, Server.Scripting.MarketFeeOperation.Commission, MarketPanelType.Auction, auction.ItemType, auction, auction.Item, cost);
                    ...
                    uint gold = feeRequest.SellerReceives;
        ...
        public void MarketGetBack(MarketCollectionMode mode, ulong auctionID)
        {
            ...
            bool TakeAuction(AuctionInfo auction, NPCObject trustMerchant)
            {
                if (auction.Sold && auction.Expired)
                {
                ...
                if (mode == MarketCollectionMode.Any || (mode == MarketCollectionMode.Sold && auction.Sold))
                {
                    ...
                    var panelType = auction.ItemType == MarketItemType.Consign ? MarketPanelType.Consign : MarketPanelType.Auction;

                    var feeRequest = ResolveMarketFee(trustMerchant, Server.Scripting.MarketFeeOperation.Commission, panelType, auction.ItemType, auction, auction.Item, cost);
            ...
            else
            {
                ...
                if (FishFound)
                {
                    ...
                    if (Envir.Random.Next(0, 100) <= getChance)
                    {
                        ...
                        var dropRateRequest = Server.Scripting.EconomyRateHooks.ResolveDropRate(this, "fishing");

                        if (dropRateRequest.Decision == Server.Scripting.ScriptHookDecision.Cancel &&
                            !string.IsNullOrEmpty(dropRateRequest.FailMessage))
                        {
                            ReceiveChat(dropRateRequest.FailMessage, ChatType.System);
                        }
                        ...
                        var fishingDropName = Settings.FishingDropFilename;
                        if (!string.IsNullOrWhiteSpace(fishingDropName))
                        {
                            var prefix = ((int)fishingCell.FishingAttribute).ToString("D2");
                            fishingDropName = fishingDropName.Replace("00", prefix);
                            dropTableKey = $"Drops/{fishingDropName.Replace('\\', '/')}";
                        }

                        var dropContext = new Server.Scripting.DropAttemptContext("fishing", this, null, dropTableKey);
                        ...
                        else if (FreeSpace(Info.Inventory) < 1)
                        {
                         ...
                         else
                         {
                             ...
                             Server.Scripting.RareDropAnnouncements.Notify(dropItem, dropContext);
                         }
        ...
        public void AcceptQuest(int index)
        {
            ...
            if (!info.TryCheckAcceptCondition(this, out var acceptFailMessage))
            {
                ReceiveChat(acceptFailMessage, ChatType.System);
                return;
            }
            ...
            quest.Info?.TryInvokeOnAccepted(this, quest);
        ...
        public void FinishQuest(int questIndex, int selectedItemIndex = -1)
        {
            ...
            if (!quest.Info.TryCheckFinishCondition(this, quest, selectedItemIndex, out var finishFailMessage))
            {
                ReceiveChat(finishFailMessage, ChatType.System);
                return;
            }

            quest.Info.ResolveRewards(this, quest, selectedItemIndex, out var rewardGold, out var rewardExp, out var rewardCredit, out var fixedRewards, out var selectRewards);
            ...
            {
                for (int i = 0; i < selectRewards.Count; i++)
                {
                    if (selectedItemIndex != i) continue;

                    ushort count = selectRewards[i].Count;
                    UserItem rewardItem;

                    while (count > 0)
                    {
                        rewardItem = Envir.CreateFreshItem(selectRewards[i].Item);
                        if (selectRewards[i].Item.StackSize >= count)
                        {
                            rewardItem.Count = count;
                            count = 0;
                        }
                        ...
                        {
                            rewardItem.Count = selectRewards[i].Item.StackSize;
                            count -= selectRewards[i].Item.StackSize;
                        }
            ...
            GainGold(rewardGold);
            GainExp(rewardExp);
            GainCredit(rewardCredit);

            quest.Info.TryInvokeOnFinished(this, quest, selectedItemIndex);
        ...
        public void SendMail(string name, string message, uint gold, ulong[] items, bool stamped)
        {
            ...
            for (int j = 0; j < (hasStamp ? 5 : 1) && j < items.Length; j++)
            {
        ...
        public uint GetMailCost(ulong[] items, uint gold, bool stamped, Server.Scripting.MailCostOperation operation = Server.Scripting.MailCostOperation.Send)
        {
            items ??= Array.Empty<ulong>();

            bool requestedStamped = stamped;
            bool effectiveStamped = false;

            if (requestedStamped)
            {
                for (int i = 0; i < Info.Inventory.Length; i++)
                {
                    UserItem stamp = Info.Inventory[i];
                    if (stamp == null) continue;

                    if (stamp.Info.Type == ItemType.杂物 && stamp.Info.Shape == 1 && stamp.Count >= 1)
                    {
                        effectiveStamped = true;
                        break;
                    }
                }
            }

            var mailItems = new List<UserItem>();

            int slots = effectiveStamped ? 5 : 1;

            if (items.Length > 0)
            {
                for (int j = 0; j < slots && j < items.Length; j++)
                {
                    if (items[j] < 1) continue;
            ...
            if (!Settings.MailFreeWithStamp || !effectiveStamped)
            {
                if (gold > 0 && Settings.MailCostPer1KGold > 0)
                {
                    baseCost += (uint)Math.Floor((decimal)gold / 1000) * Settings.MailCostPer1KGold;
                }

                if (mailItems.Count > 0 && Settings.MailItemInsurancePercentage > 0)
                {
                    for (int i = 0; i < mailItems.Count; i++)
                    {
                        baseCost += (uint)Math.Floor((double)mailItems[i].Price() / 100 * Settings.MailItemInsurancePercentage);
                    }
                }
            }

            var request = new Server.Scripting.MailCostRequest(
                this,
                operation,
                items,
                mailItems,
            ...
            var scriptsRuntimeActive = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
            var allowBefore = scriptsRuntimeActive &&
                              Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMailCostBefore) &&
                              Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMailCostBeforeOperation(operation));
            var allowAfter = scriptsRuntimeActive &&
                             Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMailCostAfter) &&
                             Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMailCostAfterOperation(operation));

            if (allowBefore)
            {
                Envir.CSharpScripts.TryHandleMailCostBefore(request);
            }

            if (allowAfter)
            {
                var result = new Server.Scripting.MailCostResult(
                    this,
                    operation,
                    items,
                    mailItems,
                ...
                Envir.CSharpScripts.TryHandleMailCostAfter(result);
            }
        ...
        public void StrongboxRewardItem(int boxtype)
        {
            ...
            var dropRateRequest = Server.Scripting.EconomyRateHooks.ResolveDropRate(this, "strongbox");

            if (dropRateRequest.Decision == Server.Scripting.ScriptHookDecision.Cancel)
            {
                if (!string.IsNullOrEmpty(dropRateRequest.FailMessage))
                    ReceiveChat(dropRateRequest.FailMessage, ChatType.System);
            ...
            {
                int rate = (int)(Envir.Random.Next(0, drop.Chance) / effectiveDropRate);
                if (rate < 1) rate = 1;
        ...
        public void BlackstoneRewardItem()
        {
            ...
            var dropRateRequest = Server.Scripting.EconomyRateHooks.ResolveDropRate(this, "blackstone");
            ...
            {
                int rate = (int)(Envir.Random.Next(0, drop.Chance) / effectiveDropRate); if (rate < 1) rate = 1;
