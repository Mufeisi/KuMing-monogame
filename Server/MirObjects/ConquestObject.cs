...
namespace Server.MirObjects
{
    public class ConquestObject
    {
        ...
        {
            return new ActivityDescriptor(ActivitySourceType.Conquest, Info.Index.ToString(), Info.Name ?? string.Empty, ConquestMap, Info.Index);
        }

        private ActivityProgressRequest TryCSharpActivityProgressBefore(ActivityProgressReason reason, int previousValue, int value, int maxValue)
        {
            ...
            var request = new ActivityProgressRequest(CreateActivityDescriptor(), ActivityProgressKind.State, reason, previousValue, value, maxValue, this);
            Envir.CSharpScripts.TryHandleActivityProgressBefore(request);
            return request;
        }

        private void TryCSharpActivityProgressAfter(ActivityProgressReason reason, int previousValue, int value, int maxValue, bool executedLegacy, ScriptHookDecision decision)
        {
            var scriptsRuntimeActive = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
            if (!scriptsRuntimeActive) return;
            if (!ScriptDispatchPolicy.ShouldTryCSharp(ScriptHookKeys.OnActivityProgressAfter)) return;

            var result = new ActivityProgressResult(CreateActivityDescriptor(), ActivityProgressKind.State, reason, previousValue, value, maxValue, executedLegacy, decision, this);
            Envir.CSharpScripts.TryHandleActivityProgressAfter(result);
        }

        private ActivityResultRequest TryCSharpActivityResultBefore(ActivityResultReason reason, GuildObject previousWinner, GuildObject winner, PlayerObject actor, bool endWar)
        {
            var scriptsRuntimeActive = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
            if (!scriptsRuntimeActive) return null;
            if (!ScriptDispatchPolicy.ShouldTryCSharp(ScriptHookKeys.OnActivityResultBefore)) return null;

            var request = new ActivityResultRequest(CreateActivityDescriptor(), reason, previousWinner, winner, actor, endWar, this);
            Envir.CSharpScripts.TryHandleActivityResultBefore(request);
            return request;
        }

        private void TryCSharpActivityResultAfter(ActivityResultReason reason, GuildObject previousWinner, GuildObject winner, PlayerObject actor, bool endedWar, bool executedLegacy, ScriptHookDecision decision)
        {
            var scriptsRuntimeActive = Settings.CSharpScriptsEnabled && Envir.CSharpScripts.Enabled;
            if (!scriptsRuntimeActive) return;
            if (!ScriptDispatchPolicy.ShouldTryCSharp(ScriptHookKeys.OnActivityResultAfter)) return;

            var result = new ActivityResult(CreateActivityDescriptor(), reason, previousWinner, winner, actor, endedWar, executedLegacy, decision, this);
            Envir.CSharpScripts.TryHandleActivityResultAfter(result);
        }
        ...
        public void TakeConquest(PlayerObject player = null, GuildObject winningGuild = null)
        {
            ...
            GuildObject requestedWinner = winningGuild ?? player?.MyGuild;
            if (requestedWinner == null) return;

            GuildObject tmpPrevious = null;
            GuildObject previousWinner = Guild;
            bool shouldEndWar = GameType == ConquestGame.占领皇宫;
            bool endedWar = false;
            bool executedLegacy = false;
            ScriptHookDecision scriptDecision = ScriptHookDecision.Continue;

            var resultRequest = TryCSharpActivityResultBefore(ActivityResultReason.Capture, previousWinner, requestedWinner, player, shouldEndWar);
            if (resultRequest != null)
            {
                requestedWinner = resultRequest.Winner;
                shouldEndWar = resultRequest.EndWar;
                scriptDecision = resultRequest.Decision;
                ...
                {
                    TryCSharpActivityResultAfter(ActivityResultReason.Capture, previousWinner, Guild, player, !WarIsOn, false, scriptDecision);
                    return;
                }
            }

            if (requestedWinner == null) return;
            if (requestedWinner.Conquest != null && requestedWinner.Conquest != this) return;

            switch (GameType)
            {
                case ConquestGame.占领皇宫:
                    if (StartType == ConquestType.申请启动)
                        if (requestedWinner.Guildindex != GuildInfo.AttackerID) break;
                    ...
                    GuildInfo.Owner = requestedWinner.Guildindex;
                    Guild = requestedWinner;
                    requestedWinner.Conquest = this;
                    if (shouldEndWar)
                    {
                        EndWar(GameType);
                        endedWar = true;
            ...
            if (executedLegacy)
            {
                TryCSharpActivityResultAfter(ActivityResultReason.Capture, previousWinner, Guild, player, endedWar || !WarIsOn, true, scriptDecision);
            }
        }
        ...
        public void StartWar(ConquestGame type)
        {
            ...
            var progressRequest = TryCSharpActivityProgressBefore(ActivityProgressReason.Start, previousState, requestedState, 1);
            if (progressRequest != null)
            {
                requestedState = progressRequest.Value > 0 ? 1 : 0;
                scriptDecision = progressRequest.Decision;
                ...
                if (scriptDecision == ScriptHookDecision.Handled)
                {
                    TryCSharpActivityProgressAfter(ActivityProgressReason.Start, previousState, WarIsOn ? 1 : 0, 1, false, scriptDecision);
                    return;
                }
            }

            WarIsOn = requestedState > 0;
            executedLegacy = true;

            foreach (var pl in Envir.Players)
                pl.BroadcastInfo();

            TryCSharpActivityProgressAfter(ActivityProgressReason.Start, previousState, WarIsOn ? 1 : 0, 1, executedLegacy, scriptDecision);
        }
        ...
        public void EndWar(ConquestGame type)
        {
            ...
            var progressRequest = TryCSharpActivityProgressBefore(ActivityProgressReason.End, previousState, requestedState, 1);
            if (progressRequest != null)
            {
                requestedState = progressRequest.Value > 0 ? 1 : 0;
                scriptDecision = progressRequest.Decision;
                ...
                if (scriptDecision == ScriptHookDecision.Handled)
                {
                    TryCSharpActivityProgressAfter(ActivityProgressReason.End, previousState, WarIsOn ? 1 : 0, 1, false, scriptDecision);
                    return;
                }
            }
            ...
            TryCSharpActivityProgressAfter(ActivityProgressReason.End, previousState, WarIsOn ? 1 : 0, 1, executedLegacy, scriptDecision);

            if (!WarIsOn)
            {
                TryCSharpActivityResultAfter(ActivityResultReason.Complete, previousWinner, Guild, null, true, executedLegacy, scriptDecision);
            }
        }
    }
}
