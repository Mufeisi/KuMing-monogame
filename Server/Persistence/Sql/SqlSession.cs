...
namespace Server.Persistence.Sql
{
    public sealed class SqlSession : IDisposable
    {
            ...
            SqlSessionOptions sessionOptions)
        {
            ...
            var session = new SqlSession(provider, dialect, connection, options.CommandTimeoutSeconds, commandMaxRetries, baseRetryDelayMs, options);
        ...
        private void InitializeConnection()
        {
            if (Provider == DatabaseProviderKind.Sqlite)
            {
                ...
                if (_options.Authority == DatabaseAuthority.Character && !string.IsNullOrWhiteSpace(_options.SqliteIdentityPath))
                    TryAttachIdentityView();
            }
        }

        private void TryAttachIdentityView()
        {
            using (var databaseList = Connection.CreateCommand())
            {
                databaseList.CommandText = "SELECT COUNT(*) FROM pragma_database_list WHERE name='identity'";
                if (Convert.ToInt32(databaseList.ExecuteScalar()) == 0)
                {
                    using var attach = Connection.CreateCommand();
                    attach.CommandText = "ATTACH DATABASE @path AS identity";
                    var parameter = attach.CreateParameter();
                    parameter.ParameterName = "@path";
                    parameter.Value = Path.GetFullPath(_options.SqliteIdentityPath);
                    attach.Parameters.Add(parameter);
                    attach.ExecuteNonQuery();
                }
            }

            using var probe = Connection.CreateCommand();
            probe.CommandText = "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='account_wallets'";
            ...
            view.CommandText =
                ...
                "SELECT a.account_id,a.account_name,a.password_hash,a.password_salt,a.require_password_change," +
                "a.user_name,a.birth_utc_ms,a.secret_question,a.secret_answer,a.email_address,a.creation_ip," +
                "a.creation_utc_ms,a.banned,a.ban_reason,a.expiry_utc_ms,a.last_ip,a.last_utc_ms," +
                "a.wrong_password_count,a.admin_account,COALESCE(w.gold,0) AS gold,COALESCE(w.credit,0) AS credit,a.updated_utc_ms " +
                "FROM identity.accounts a LEFT JOIN main.account_wallets w ON w.account_id=a.account_id";
