...
namespace Server.MirEnvir
{
    ...
    public class Envir
    {
        ...
        private IGamePersistence Persistence => _persistence ??= ServerPersistenceFactory.CreateFromSettings(this);
        ...
        private void WorkLoop()
        {
            try
            {
                ...
                StartEnvir();
                if (Persistence.State is not PersistenceModuleState.Loading and not PersistenceModuleState.Ready)
                {
                    MessageQueue.Enqueue($"持久化基础加载失败，拒绝继续启动。当前状态：{Persistence.State}");
                    StopEnvir();
                    _thread = null;
                ...
                try
                {
                    while (Running)
                    {
                        ...
                        if (Time >= saveTime || forceAutoSave)
                        {
                            ...
                            TryAutoSave(SqlSaveDomain.Accounts, () =>
                            {
                                var result = Persistence.Commit(CheckpointKind.CharacterRuntime, CommitReason.AutoSave);
                                if (!result.Committed)
                                    throw new InvalidOperationException($"Character checkpoint 失败：{result.ErrorCode} {result.Diagnostics}");
                            });
                        }
                ...
                StopNetwork();
                StopEnvir();
                var shutdownCommit = Persistence.Commit(CheckpointKind.CharacterRuntime, CommitReason.Shutdown);
                if (!shutdownCommit.Committed)
                    MessageQueue.Enqueue($"关服 Character checkpoint 失败：{shutdownCommit.ErrorCode} {shutdownCommit.Diagnostics}");
            }
            catch (Exception ex)
            {
                try
        ...
        public void SaveDB()
        {
            var result = Persistence.Commit(CheckpointKind.WorldDefinition, CommitReason.EditorSave);
            if (!result.Committed)
                throw new InvalidOperationException($"World Definition 保存失败：{result.ErrorCode} {result.Diagnostics}");
        }

        public CharacterInfo GetArchivedCharacter(string name)
        {
            return Persistence.ExecuteCharacter(new LoadCharacterBackupCommand(name)).Character;
        }

        public CharacterResult SaveArchivedCharacter(CharacterInfo info)
        {
            return Persistence.ExecuteCharacter(new BackupCharacterCommand(info));
        }

        public CharacterResult ArchiveCharacter(CharacterInfo info)
        {
            return Persistence.ExecuteCharacter(new ArchiveCharacterCommand(info));
        }

        public CharacterResult RestoreCharacter(string name, AccountInfo account)
        {
            return Persistence.ExecuteCharacter(new RestoreCharacterCommand(name, account));
        }

        public CommitResult SaveAccounts()
        {
            return Persistence.Commit(CheckpointKind.CharacterRuntime, CommitReason.Operator);
        }

        private void SaveGoods(bool forced = false)
        {
            var result = Persistence.Commit(CheckpointKind.CharacterRuntime, forced ? CommitReason.Shutdown : CommitReason.Operator);
            if (!result.Committed)
                throw new InvalidOperationException($"Character Runtime 保存失败：{result.ErrorCode} {result.Diagnostics}");
        }
        ...
        public void BeginSaveAccounts()
        {
            var result = Persistence.Commit(CheckpointKind.CharacterRuntime, CommitReason.AutoSave);
            if (!result.Committed)
                throw new InvalidOperationException($"Character Runtime 自动保存失败：{result.ErrorCode} {result.Diagnostics}");
        }
        ...
        public bool LoadDB()
        {
            ...
            if (!result.Committed)
                MessageQueue.Enqueue($"持久化启动加载失败：{result.ErrorCode} {result.Diagnostics}");
        ...
        public void LoadAccounts()
        {
            var result = Persistence.LoadStartup();
            if (!result.Committed)
                throw new InvalidOperationException($"持久化启动加载失败：{result.ErrorCode} {result.Diagnostics}");
        }

        public void LoadGuilds()
        {
            var result = Persistence.ExecuteCharacter(new LoadGuildRuntimeCommand());
            if (!result.Committed)
                throw new InvalidOperationException($"Guild Runtime 加载失败：{result.ErrorCode} {result.Diagnostics}");
        }

        public void LoadGoods()
        {
            var result = Persistence.ExecuteCharacter(new LoadNpcGoodsRuntimeCommand());
            if (!result.Committed)
                throw new InvalidOperationException($"NPC Goods Runtime 加载失败：{result.ErrorCode} {result.Diagnostics}");
        }

        public void LoadConquests()
        {
            var result = Persistence.ExecuteCharacter(new LoadConquestRuntimeCommand());
            if (!result.Committed)
                throw new InvalidOperationException($"Conquest Runtime 加载失败：{result.ErrorCode} {result.Diagnostics}");
        }
        ...
        private void StartNetwork()
        {
            ...
            if (Persistence.State != PersistenceModuleState.Ready)
                throw new InvalidOperationException($"持久化完整启动未进入 Ready，拒绝开放网络。当前状态：{Persistence.State}");
        ...
        public void NewAccount(ClientPackets.NewAccount p, MirConnection c)
        {
            ...
            lock (AccountLock)
            {
                ...
                var created = new AccountInfo(p) { Index = ++NextAccountID, CreationIP = c.IPAddress };
                AccountList.Add(created);
                var identity = Persistence.ExecuteIdentity(new PersistIdentitySnapshotCommand());
                if (!identity.Committed)
                {
                    AccountList.Remove(created);
                    MessageQueue.Enqueue($"创建账号 Identity 提交失败：{identity.ErrorCode} {identity.Diagnostics}");
                    c.Enqueue(new ServerPackets.NewAccount { Result = 0 });
                    return;
                }
        ...
        public int HTTPNewAccount(ClientPackets.NewAccount p, string ip)
        {
            ...
            lock (AccountLock)
            {
                ...
                var created = new AccountInfo(p) { Index = ++NextAccountID, CreationIP = ip };
                AccountList.Add(created);
                var identity = Persistence.ExecuteIdentity(new PersistIdentitySnapshotCommand());
                if (!identity.Committed)
                {
                    AccountList.Remove(created);
                    MessageQueue.Enqueue($"HTTP 创建账号 Identity 提交失败：{identity.ErrorCode} {identity.Diagnostics}");
                    return 0;
                }
                return 8;
            }
        ...
        public void ChangePassword(ClientPackets.ChangePassword p, MirConnection c)
        {
            ...
            var passwordCommit = Persistence.ExecuteIdentity(new PersistIdentitySnapshotCommand());
            if (!passwordCommit.Committed)
            {
                c.Enqueue(new ServerPackets.ChangePassword { Result = 0 });
                MessageQueue.Enqueue($"修改密码 Identity 提交失败：{passwordCommit.ErrorCode} {passwordCommit.Diagnostics}");
                return;
            }
        ...
        public void Login(ClientPackets.Login p, MirConnection c)
        {
            ...
            if (string.CompareOrdinal(account.Password, p.Password) != 0)
            {
                if (account.WrongPasswordCount++ >= 5)
                {
                    ...
                    Persistence.ExecuteIdentity(new PersistIdentitySnapshotCommand());
            ...
            var loginCommit = Persistence.ExecuteIdentity(new PersistIdentitySnapshotCommand());
            if (!loginCommit.Committed)
            {
                c.SendDisconnect(3);
                MessageQueue.Enqueue($"登录结果 Identity 提交失败：{loginCommit.ErrorCode} {loginCommit.Diagnostics}");
                return;
            }
        ...
        public int HTTPLogin(string AccountID, string Password)
        {
            ...
            account.WrongPasswordCount = 0;
            if (!Persistence.ExecuteIdentity(new PersistIdentitySnapshotCommand()).Committed)
                return 0;
            return 7;
        }
