...
public sealed class CrashDiagnosticBundleTests
{
    ...
    public void 原子诊断包包含日志尾部版本资源版本和配置摘要()
    {
        ...
        try
        {
            ...
            File.WriteAllText(log, new string('x', 70_000) +
                "-tail-proof token=supersecret user@example.com 192.168.1.10 " +
                @"Authorization: Bearer signed.payload C:\Users\private-user\client.log");
