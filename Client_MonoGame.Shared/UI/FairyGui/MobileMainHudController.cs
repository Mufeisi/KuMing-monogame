...
namespace MonoShare
{
    internal sealed class MobileMainHudController : IDisposable
    {
        ...
        private bool Bind(InIReader reader, ISet<GObject> usedTargets, string actionKey, string actionName, string[] defaultKeywords, Action action)
        {
            ...
            // Mentor/Mentee is a required Android entry point even when the
            // published HUD package has no matching FUI object.  Reuse this
            // controller seam to add one reachable, disposable fallback
            // button; normal package targets still win when present.
            if (target == null &&
            ...
            if (target == null &&
                string.Equals(actionKey, "Fishing", StringComparison.OrdinalIgnoreCase))
            {
                target = TryCreateFishingHudFallbackButton();
                if (target != null)
                {
                    selectedScore = Math.Max(selectedScore, 1000);
                    error = null;
        ...
        private GObject TryCreateMentorHudFallbackButton()
        {
            if (_root == null || _root._disposed)
                return null;
            ...
            try
            {
                GObject existing = _root.GetChild("__codex_mobile_mentor_hud_fallback");
            ...
            try
            {
                ...
                {
                    name = "__codex_mobile_mentor_hud_fallback_bg",
                    touchable = false,
                };
        ...
        private GObject TryCreateFishingHudFallbackButton()
        {
            if (_root == null || _root._disposed)
                return null;
            ...
            try
            {
                GObject existing = _root.GetChild("__codex_mobile_fishing_hud_fallback");
                if (existing != null && !existing._disposed)
                    return existing;
            }
            catch
            ...
            {
                MobileMainHudFallbackBounds bounds =
                    MobileMainHudFallbackLayout.Fishing(_root.width, _root.height);
                float width = bounds.Width;
                float height = bounds.Height;
                var button = new GButton
                {
                    name = "__codex_mobile_fishing_hud_fallback",
                    title = "钓鱼",
                    touchable = true,
                    enabled = true,
                    grayed = false,
                ...
                var background = new GGraph
                {
                    name = "__codex_mobile_fishing_hud_fallback_bg",
                    touchable = false,
                };
                background.DrawRect(width, height, 2,
                    new Color(90, 155, 170, 255),
                    new Color(32, 78, 92, 245));
                button.AddChild(background);
        ...
        private GObject TryCreateSealRentalHudFallbackButton()
        {
            if (_root == null || _root._disposed)
                return null;
            ...
            try
            {
                GObject existing = _root.GetChild("__codex_mobile_seal_rental_hud_fallback");
                if (existing != null && !existing._disposed)
                    return existing;
            }
            catch
            ...
            {
                MobileMainHudFallbackBounds bounds =
                    MobileMainHudFallbackLayout.SealRental(_root.width, _root.height);
                float width = bounds.Width;
                float height = bounds.Height;
                var button = new GButton
                {
                    name = "__codex_mobile_seal_rental_hud_fallback",
                    title = "封印/租赁",
                    touchable = true,
                    enabled = true,
                    grayed = false,
                ...
                var background = new GGraph
                {
                    name = "__codex_mobile_seal_rental_hud_fallback_bg",
                    touchable = false,
                };
                background.DrawRect(width, height, 2,
                    new Color(175, 140, 100, 255), new Color(92, 60, 40, 245));
                button.AddChild(background);
