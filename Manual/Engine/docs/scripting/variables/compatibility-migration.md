...
- 部署状态：指定项目快照已完成预检；脚本变化后必须重新审核摘要
...
输出包含扫描根目录、TXT/INI/CS 文件数量、SHA-256 内容摘要、固定前缀引用数量、只读系统占位符清单以及逐文件诊断。退出码含义：
...
| `VAR07-CONTENT-001` | 错误 | 没有可扫描的 TXT/INI/CS 内容 |
...
| `VAR08-SENSITIVE-001` | 错误 | 请求密码、密保、电话或邮箱等禁止暴露的信息 |
| `VAR08-UNKNOWN-001` | 错误 | 占位符未进入当前真实内容审核清单 |
...
指定目录 `Server-mono/Envir` 的只读快照已经形成可重复门禁：

| 项目 | 审核结果 |
|---|---|
| 文件数 | 2348 个 TXT/INI/CS 文件 |
| 内容摘要 | `7E1E13532F37151BBC15E4B7B43383A8C1D95E3B0DC25944A2F19864D74ABE3D` |
| 固定变量 | A 36 次、D 36 次 |
| 只读占位符 | 47 种，全部可由现有解析器处理 |
| 阻断错误 | 0 |
| A 审核 | 28 次读、8 次写，全部位于“定位符咒”的两份等价生成内容 |

旧 `LOADVALUE/SAVEVALUE/%A*` 入口已经与 `MOV A*` 统一到新的全服持久 A 存储，进程冒烟验证不会再落回人物 `NPCVar`。这只保证存储语义一致；A 仍是全服共享变量，脚本作者不得把它当私人临时槽位。52 个换行警告不改变执行语义，但内容重新生成时建议统一为 CRLF。

该摘要只绑定上述快照。任何文件数量、内容、变量或占位符变化都会使真实项目回归测试失败，必须重新扫描并形成新的审核记录。
