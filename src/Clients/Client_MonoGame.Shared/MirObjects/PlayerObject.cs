...
namespace MonoShare.MirObjects
{
    public class PlayerObject : MapObject
    {
        ...
        private MLibrary _lastHairLibraryRef;
        private int _lastValidHairDrawIndex = -1;
            ...
            else
            {
                ...
                switch (Class)
                {
                    ...
                    case MirClass.Archer:
                        ...
                        try
                        {
                            // 移动端资源按需下载：运行时可能只初始化了 00.Lib，需按实际索引动态扩容
                            Libraries.EnsureARArmourLibraryIndex(Armour);
                            Libraries.EnsureCArmourLibraryIndex(Armour);
                            Libraries.EnsureARHairLibraryIndex(Hair);
                            Libraries.EnsureCHairLibraryIndex(Hair);

                            if (HasClassWeapon)
                            {
                                int index = Weapon - 200;
                                Libraries.EnsureARWeaponLibraryIndex(index);
                                Libraries.EnsureARWeaponsSLibraryIndex(index);
                            }
                            else if (Weapon >= 0)
                            {
                                Libraries.EnsureCWeaponLibraryIndex(Weapon);
                                if (WeaponEffect > 0)
                                    Libraries.EnsureCWeaponEffectLibraryIndex(WeaponEffect);
                            }

                            if (WingEffect > 0 && WingEffect < 100)
                            {
                                int effectIndex = WingEffect - 1;
                                Libraries.EnsureARHumEffectLibraryIndex(effectIndex);
                                Libraries.EnsureCHumEffectLibraryIndex(effectIndex);
                            }
                        }
                        catch
                        {
                    ...
                    case MirClass.Assassin:
                        ...
                        {
                            // 移动端资源按需下载：运行时可能只初始化了 00.Lib，需按实际索引动态扩容
                            Libraries.EnsureAArmourLibraryIndex(Armour);
                            Libraries.EnsureCArmourLibraryIndex(Armour);
                            Libraries.EnsureAHairLibraryIndex(Hair);
                            Libraries.EnsureCHairLibraryIndex(Hair);

                            if (HasClassWeapon)
                            {
                                int index = Weapon - 100;
                                Libraries.EnsureAWeaponLLibraryIndex(index);
                                Libraries.EnsureAWeaponRLibraryIndex(index);
                            }
                            else if (Weapon >= 0)
                            {
                                Libraries.EnsureCWeaponLibraryIndex(Weapon);
                            ...
                            {
                                int effectIndex = WingEffect - 1;
                                Libraries.EnsureAHumEffectLibraryIndex(effectIndex);
                                Libraries.EnsureCHumEffectLibraryIndex(effectIndex);
                            }
                        }
                        catch
                    ...
                    case MirClass.Wizard:
                        ...
                        try
                        {
                            ...
                            if (WingEffect > 0 && WingEffect < 100)
                                Libraries.EnsureCHumEffectLibraryIndex(WingEffect - 1);
                        }
                        catch
                        {
                        }
        ...
        public void PlayStepSound()
        {
            ...
            if (mapControl?.M2CellInfo == null)
                return;

            if (x < 0 || y < 0 || x >= mapControl.Width || y >= mapControl.Height)
                return;

            if (mapControl.M2CellInfo[x, y] == null)
                return;
        ...
        {
            MLibrary library = HairLibrary;
            if (library == null)
            {
                _lastHairLibraryRef = null;
                _lastValidHairDrawIndex = -1;
                return;
            }

            if (!ReferenceEquals(_lastHairLibraryRef, library))
            {
                _lastHairLibraryRef = library;
                _lastValidHairDrawIndex = -1;
            }

            int index = DrawFrame + HairOffSet;
            if (library.Touch(index))
            {
                _lastValidHairDrawIndex = index;
                library.Draw(index, DrawLocation, DrawColour, true);
                return;
            }

            int fallback = _lastValidHairDrawIndex;
            if (fallback >= 0 && library.Touch(fallback))
            {
                library.Draw(fallback, DrawLocation, DrawColour, true);
                return;
            }

            // 兜底：尝试相邻帧，避免“隔帧缺图”导致的头发闪烁
            for (int delta = 1; delta <= 2; delta++)
            {
                int left = index - delta;
                if (left >= 0 && library.Touch(left))
                {
                    _lastValidHairDrawIndex = left;
                    library.Draw(left, DrawLocation, DrawColour, true);
                    return;
                }

                int right = index + delta;
                if (library.Touch(right))
                {
                    _lastValidHairDrawIndex = right;
                    library.Draw(right, DrawLocation, DrawColour, true);
                    return;
                }
            }
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(Name) && string.IsNullOrWhiteSpace(GuildName))
                return;
            ...
            DrawNameBlock(lines, _ => NameColour, yOffset: Dead ? 35 : 8);
        }
