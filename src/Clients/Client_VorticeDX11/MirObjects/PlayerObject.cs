    ...
    public class PlayerObject : MapObject
    {
        ...
        public override void Process()
        {
            ...
            switch (CurrentAction)
            {
                ...
                case MirAction.刺客冲击:
                    ...
                    bool useMoveTick = CurrentAction != MirAction.弓箭跳跃;
                    int moveTickInterval = Poison.HasFlag(PoisonType.Slow) ? 200 : 100;
                    float moveProgress = GetMoveFrameProgress(count, index, FrameInterval, useMoveTick, moveTickInterval);
                    OffSetMove = GetDirectionalMoveOffset(Direction, i, count, index, FrameInterval, useMoveTick, moveTickInterval);

                    if (ShouldRedrawSmoothMovement(FrameInterval, useMoveTick, count, moveTickInterval))
                    {
                    ...
                    if (this == User && Settings.SmoothMove)
                    {
                        Client.Utils.ResolutionTrace.LogThrottled(
                            "SmoothMove.Player",
                            "UserMove",
                            50,
                            $"Action={CurrentAction}, Dir={Direction}, Current={CurrentLocation.X},{CurrentLocation.Y}, Movement={Movement.X},{Movement.Y}, FrameIndex={FrameIndex}, FrameCount={count}, Progress={moveProgress:F3}, Offset={OffSetMove.X},{OffSetMove.Y}, MotionStart={MotionStartTime}, Now={CMain.Time}, CanMove={GameScene.CanMove}, MoveTime={GameScene.MoveTime}");
                    }
                    break;
                default:
                    OffSetMove = Point.Empty;
            ...
            DrawLocation = new Point((Movement.X - User.Movement.X + MapControl.OffSetX) * MapControl.CellWidth + MapControl.ViewOffsetX, (Movement.Y - User.Movement.Y + MapControl.OffSetY) * MapControl.CellHeight + MapControl.ViewOffsetY);
        ...
        public virtual void SetAction()
        {
            ...
            else
            {
                ...
                FrameInterval = Frame.Interval;
                EffectFrameInterval = Frame.EffectInterval;
                MotionStartTime = CurrentAction switch
                {
                    MirAction.行走动作 => CMain.Time,
                    MirAction.跑步动作 => CMain.Time,
                    MirAction.坐骑行走 => CMain.Time,
                    MirAction.坐骑奔跑 => CMain.Time,
                    MirAction.推开动作 => CMain.Time,
                    MirAction.左冲动作 => CMain.Time,
                    MirAction.右冲动作 => CMain.Time,
                    MirAction.刺客潜行 => CMain.Time,
                    _ => 0,
                };

                if (this == User && MotionStartTime > 0 && Settings.SmoothMove)
                {
                    Client.Utils.ResolutionTrace.Log(
                        "SmoothMove.PlayerAction",
                        $"Start Action={CurrentAction}, Dir={Direction}, Location={CurrentLocation.X},{CurrentLocation.Y}, FrameInterval={FrameInterval}, EffectFrameInterval={EffectFrameInterval}, Sprint={Sprint}, Slow={Poison.HasFlag(PoisonType.Slow)}, MotionStart={MotionStartTime}");
                }
