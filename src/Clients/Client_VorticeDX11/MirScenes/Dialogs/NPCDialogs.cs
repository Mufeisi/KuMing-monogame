    ...
    public sealed class NPCGoodsDialog : MirImageControl
    {
        ...
        private void BuyItem()
        {
            ...
            if (SelectedItem.Info.StackSize > 1)
            {
                ...
                uint totalPrice = (uint)(SelectedItem.Price() * GameScene.NPCRate);
                uint unitPrice = Math.Max(1U, (uint)Math.Ceiling((SelectedItem.Price() / (float)Math.Max(1, (int)SelectedItem.Count)) * GameScene.NPCRate));

                if (UsePearls)
                {
                    if (totalPrice > GameScene.User.PearlCount)
                    {
                        maxQuantity = Math.Min(ushort.MaxValue, (ushort)(GameScene.User.PearlCount / unitPrice));
                        if (maxQuantity == 0)
                        {
                ...
                else if (totalPrice > GameScene.Gold)
                {
                    maxQuantity = Math.Min(ushort.MaxValue, (ushort)(GameScene.Gold / unitPrice));
                    if (maxQuantity == 0)
                    {
            ...
            {
                uint singlePrice = (uint)(SelectedItem.Price() * GameScene.NPCRate);

                if (UsePearls ? singlePrice > GameScene.User.PearlCount : singlePrice > GameScene.Gold)
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(UsePearls ? "没有足够的珍珠" : GameLanguage.LowGold, ChatType.System);
                    return;
                }
    ...
    public sealed class NPCDropDialog : MirImageControl
    {
        ...
        private void Confirm()
        {
            ...
            switch (PType)
            {
                case PanelType.Sell:
                    ...
                    if ((ulong)GameScene.Gold + (uint)(TargetItem.Price() * GameScene.NPCSellRate) <= uint.MaxValue)
                    {
        ...
        private void NPCDropPanel_BeforeDraw(object sender, EventArgs e)
        {
            ...
            if (TargetItem != null)
            {
                ...
                {
                    case PanelType.Sell:
                        text += ((uint)(TargetItem.Price() * GameScene.NPCSellRate)).ToString();
                        break;
                    case PanelType.Repair:
