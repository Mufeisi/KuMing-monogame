...
namespace Client.MirScenes
{
    public sealed class GameScene : MirScene
    {
        ...
        private const int ViewZoomStepPercent = 1;

        private bool TryHandleViewZoomHotkey(KeyEventArgs e)
        {
            if (MapControl == null || MapControl.IsDisposed) return false;
            if (Program.Form?.ActiveControl is TextBox) return false;
            if (CMain.Alt || CMain.Ctrl) return false;
        ...
        public GameScene()
        {
            CMain.SaveError(DXManager.PrintParentMethod() + $"GameScene开始初始化");
            Client.Utils.ResolutionTrace.LogClientState("GameScene.ctor", "Begin");
            ...
            UIManager.ApplyCurrentProfile(this, save: false);
        }
            ...
            foreach (KeyBind KeyCheck in CMain.InputKeys.Keylist)
            {
                ...
                switch (KeyCheck.function)
                {
                    ...
                    case KeybindOptions.Closeall:
                        ...
                        OptionDialog.Hide();
                        MenuDialog.Hide();
                        if (NPCDialog.Visible) NPCDialog.CloseConversation();
                        HelpDialog.Hide();
                    ...
                    case KeybindOptions.Options2:
                        ...
                        break;
                    case KeybindOptions.ToggleUIProfile:
                        UIManager.CycleNext(this, save: true);
                        break;
        ...
        public override void Process()
        {
            ...
            if (CMain.Time >= MoveTime)
            {
                ...
                ResolutionTrace.LogThrottled(
                    "SmoothMove.Tick",
                    "MoveTick",
                    50,
                    $"Now={CMain.Time}, MoveTime={MoveTime}, UserAction={User?.CurrentAction}, UserCurrent={User?.CurrentLocation.X},{User?.CurrentLocation.Y}, UserMovement={User?.Movement.X},{User?.Movement.Y}, UserOffset={User?.OffSetMove.X},{User?.OffSetMove.Y}");
            }
            else
                CanMove = false;
    ...
    public sealed class MapControl : MirControl
    {
        ...
        public static int ViewPixelWidth { get; private set; } = Settings.ScreenWidth;
        public static int ViewPixelHeight { get; private set; } = Settings.ScreenHeight;
        public static int ViewOffsetX { get; private set; }
        public static int ViewOffsetY { get; private set; }
        ...
        {
            get { return GameScene.User == null ? Point.Empty : new Point((MouseLocation.X - ViewOffsetX) / CellWidth - OffSetX, (MouseLocation.Y - ViewOffsetY) / CellHeight - OffSetY).Add(GameScene.User.CurrentLocation); }
        }

        public static Point ToMouseLocation(Point p)
        {
            return new Point((p.X - MapObject.User.Movement.X + OffSetX) * CellWidth + ViewOffsetX, (p.Y - MapObject.User.Movement.Y + OffSetY) * CellHeight + ViewOffsetY).Add(MapObject.User.OffSetMove);
        }
        ...
        private Vortice.Direct3D11.ID3D11Texture2D _litSceneTexture;
        private bool _zoomSavePending;
        private long _zoomSaveTime;
        ...
        private static float ZoomFactor => Math.Clamp(Settings.ViewZoomPercent / 100F, Settings.MinViewZoomPercent / 100F, Settings.MaxViewZoomPercent / 100F);

        private static int ComputeViewPixelWidth()
        {
            return Math.Max(CellWidth * 8, (int)Math.Ceiling(Settings.ScreenWidth / ZoomFactor));
        }

        private static int ComputeViewPixelHeight()
        {
            return Math.Max(CellHeight * 8, (int)Math.Ceiling(Settings.ScreenHeight / ZoomFactor));
        }

        private void UpdateViewMetrics()
        {
            ViewPixelWidth = ComputeViewPixelWidth();
            ViewPixelHeight = ComputeViewPixelHeight();

            OffSetX = ViewPixelWidth / 2 / CellWidth;
            OffSetY = ViewPixelHeight / 2 / CellHeight - 1;
            ViewOffsetX = (int)Math.Round((ViewPixelWidth / 2F) - (OffSetX * CellWidth + CellWidth / 2F));
            ViewOffsetY = (int)Math.Round((ViewPixelHeight / 2F) - (OffSetY * CellHeight + CellHeight / 2F));
        ...
        private void EnsureViewMetrics()
        {
            if (Size.Width != Settings.ScreenWidth || Size.Height != Settings.ScreenHeight ||
                ViewPixelWidth != ComputeViewPixelWidth() || ViewPixelHeight != ComputeViewPixelHeight())
            {
                UpdateViewMetrics();
            }
        }

        private static Point ClampViewPoint(Point point)
        {
            int maxX = Math.Max(0, ViewPixelWidth - 1);
            int maxY = Math.Max(0, ViewPixelHeight - 1);

            return new Point(Math.Clamp(point.X, 0, maxX), Math.Clamp(point.Y, 0, maxY));
        }

        public static Point ScreenToView(Point point)
        {
            if (point.X < 0 || point.Y < 0) return Point.Empty;
            if (Settings.ScreenWidth <= 0 || Settings.ScreenHeight <= 0) return point;
            if (ViewPixelWidth == Settings.ScreenWidth && ViewPixelHeight == Settings.ScreenHeight) return ClampViewPoint(point);

            int x = (int)Math.Round(point.X * (ViewPixelWidth / (float)Settings.ScreenWidth));
            int y = (int)Math.Round(point.Y * (ViewPixelHeight / (float)Settings.ScreenHeight));

            return ClampViewPoint(new Point(x, y));
        }

        public void ChangeZoom(int deltaPercent)
        {
            int current = Math.Clamp(Settings.ViewZoomPercent, Settings.MinViewZoomPercent, Settings.MaxViewZoomPercent);
            int next = Math.Clamp(current + deltaPercent, Settings.MinViewZoomPercent, Settings.MaxViewZoomPercent);

            if (next == current)
            {
                GameScene.Scene?.ChatDialog?.ReceiveChat(next >= Settings.MaxViewZoomPercent ? "[视角已达最大]" : "[视角已达最小]", ChatType.Hint);
                return;
            }

            Settings.ViewZoomPercent = next;
            UpdateViewMetrics();
            UpdateWeather();
            _zoomSavePending = true;
            _zoomSaveTime = CMain.Time + 300;

            GameScene.Scene?.ChatDialog?.ReceiveChat($"[视角缩放: {Settings.ViewZoomPercent}%]", ChatType.Hint);
        }

        private void FlushZoomSettings()
        {
        ...
        private void DisposeRenderTexture(ref Vortice.Direct3D11.ID3D11Texture2D texture)
        {
            if (texture == null) return;

            DXManager.ReleaseTargetBitmap(texture);
            texture.Dispose();
            texture = null;
        }
        ...
        public MapControl()
        {
            ...
            MouseMove += (o, e) => MouseLocation = ScreenToView(CMain.MPoint);
            Click += OnMouseClick;
            Client.Utils.ResolutionTrace.Log("MapControl.ctor", $"Screen={Settings.ScreenWidth}x{Settings.ScreenHeight}, View={ViewPixelWidth}x{ViewPixelHeight}, Size={Size.Width}x{Size.Height}, OffSet=({OffSetX},{OffSetY}), ViewOffset=({ViewOffsetX},{ViewOffsetY}), ViewRange=({ViewRangeX},{ViewRangeY}), Zoom={Settings.ViewZoomPercent}%");
        }
        ...
        {
            EnsureViewMetrics();
            MouseLocation = ScreenToView(CMain.MPoint);

            if (_zoomSavePending && CMain.Time >= _zoomSaveTime)
                FlushZoomSettings();

            MicroLibraryHelper.FlushPendingNotifications(message =>
            {
                if (GameScene.Scene?.ChatDialog == null || string.IsNullOrWhiteSpace(message))
                    return;
        ...
        protected override void CreateTexture()   //由 MapControl.DrawControl() 触发
        {
            //CMain.SaveError(DXManager.PrintParentMethod());
            ...
            if (Settings.SmoothMove && User.OffSetMove != Point.Empty)
            {
                ResolutionTrace.LogThrottled(
                    "SmoothMove.MapControl",
                    "CreateTexture",
                    50,
                    $"FloorValid={FloorValid}, TextureValid={TextureValid}, UserAction={User.CurrentAction}, UserCurrent={User.CurrentLocation.X},{User.CurrentLocation.Y}, UserMovement={User.Movement.X},{User.Movement.Y}, UserOffset={User.OffSetMove.X},{User.OffSetMove.Y}, AnimationCount={AnimationCount}");
            }
            ...
            {
                DisposeTexture();
                DisposeRenderTexture(ref _sceneTexture);
                DisposeRenderTexture(ref _litSceneTexture);
            }
            ...
            if (_sceneTexture == null || _sceneTexture.Description.Width != ViewPixelWidth || _sceneTexture.Description.Height != ViewPixelHeight)
            {
                DisposeRenderTexture(ref _sceneTexture);
                _sceneTexture = DXManager.NewTexture_RenderTarget_Default((uint)ViewPixelWidth, (uint)ViewPixelHeight);
            }
            ...
            if (FloorValid)
            {
                DXManager.Draw(DXManager.FloorTexture, new Rectangle(0, 0, ViewPixelWidth, ViewPixelHeight), Vector3.Zero, Color.White);
            }

            DrawObjects();

            // render weather in world space so it follows the current zoomed viewport
            ...
            var worldTexture = _sceneTexture;

            if (applyLights && DXManager.LightTexture != null)
            {
                if (_litSceneTexture == null || _litSceneTexture.Description.Width != ViewPixelWidth || _litSceneTexture.Description.Height != ViewPixelHeight)
                {
                    DisposeRenderTexture(ref _litSceneTexture);
                    _litSceneTexture = DXManager.NewTexture_RenderTarget_Default((uint)ViewPixelWidth, (uint)ViewPixelHeight);
                }

                var litSurface = DXManager.GetSurfaceLevel(_litSceneTexture, 0);
                DXManager.SetSurface(ref litSurface);
                DXManager.DeviceClear_Target(BackColour);
                DXManager.DrawMultiply(_sceneTexture, DXManager.LightTexture);
                DXManager.Sprite_Flush();
                litSurface.Dispose();
                worldTexture = _litSceneTexture;
            }

            // 2) Draw world-space hover labels to the zoomed world texture
            var worldSurface = DXManager.GetSurfaceLevel(worldTexture, 0);
            DXManager.SetSurface(ref worldSurface);

            if (applyLights && worldTexture == _litSceneTexture)
            ...
            // 3) Scale world texture into final control texture
            DXManager.SetSurface(ref controlSurface);
            DXManager.DeviceClear_Target(BackColour);
            DXManager.Draw(worldTexture, new Rectangle(0, 0, ViewPixelWidth, ViewPixelHeight), new Rectangle(0, 0, Settings.ScreenWidth, Settings.ScreenHeight), Color.White);
            DXManager.Sprite_Flush();
        ...
        {
            if (DXManager.FloorTexture == null ||
                DXManager.FloorTexture.Description.Width != ViewPixelWidth ||
                DXManager.FloorTexture.Description.Height != ViewPixelHeight)
            {
                DXManager.FloorSurface?.Dispose();
                DXManager.FloorSurface = null;
                DisposeRenderTexture(ref DXManager.FloorTexture);
                DXManager.FloorTexture = DXManager.NewTexture_RenderTarget_Default((uint)ViewPixelWidth, (uint)ViewPixelHeight);
            }
            ...
            for (int y = User.Movement.Y - ViewRangeY; y <= User.Movement.Y + ViewRangeY; y++)
            {
                ...
                if (y >= Height) break;
                drawY = (y - User.Movement.Y + OffSetY) * CellHeight + User.OffSetMove.Y + ViewOffsetY; //Moving OffSet

                for (int x = User.Movement.X - ViewRangeX; x <= User.Movement.X + ViewRangeX; x++)
                {
                    if (x <= 0 || x % 2 == 1) continue;
                    if (x >= Width) break;
                    drawX = (x - User.Movement.X + OffSetX) * CellWidth - OffSetX + User.OffSetMove.X + ViewOffsetX; //Moving OffSet
        ...
        private void DrawObjects()
        {
            ...
            for (int y = User.Movement.Y - ViewRangeY; y <= User.Movement.Y + ViewRangeY + 25; y++)
            {
                if (y <= 0) continue;
                if (y >= Height) break;
                int drawY = (y - User.Movement.Y + OffSetY + 1) * CellHeight + User.OffSetMove.Y + ViewOffsetY;

                for (int x = User.Movement.X - ViewRangeX; x <= User.Movement.X + ViewRangeX; x++)
                {
                    if (x < 0) continue;
                    if (x >= Width) break;
                    int drawX = (x - User.Movement.X + OffSetX) * CellWidth - OffSetX + User.OffSetMove.X + ViewOffsetX;
                    int index;
                    int backIndex;
                    byte animation;
                    bool blend;
        ...
        private void DrawLights(LightSetting setting)
        {
            ...
            if (DXManager.LightTexture == null ||
                DXManager.LightTexture.Description.Width != ViewPixelWidth ||
                DXManager.LightTexture.Description.Height != ViewPixelHeight)
            {
                DXManager.LightSurface?.Dispose();
                DXManager.LightSurface = null;
                DisposeRenderTexture(ref DXManager.LightTexture);
                DXManager.LightTexture = DXManager.NewTexture_RenderTarget_Default((uint)ViewPixelWidth, (uint)ViewPixelHeight);
            }
            ...
            DXManager.DeviceClear_Target(darkness);
            ...
            for (int y = MapObject.User.Movement.Y - ViewRangeY - 24; y <= MapObject.User.Movement.Y + ViewRangeY + 24; y++)
            {
                ...
                for (int x = MapObject.User.Movement.X - ViewRangeX - 24; x < MapObject.User.Movement.X + ViewRangeX + 24; x++)
                {
                    ...
                    p = new Point(
                        (x + OffSetX - MapObject.User.Movement.X) * CellWidth + MapObject.User.OffSetMove.X + ViewOffsetX,
                        (y + OffSetY - MapObject.User.Movement.Y) * CellHeight + MapObject.User.OffSetMove.Y + ViewOffsetY + 32);
        ...
        {
            Point p = new Point((MouseLocation.X - ViewOffsetX) / CellWidth, (MouseLocation.Y - ViewOffsetY) / CellHeight);
            if (Functions.InRange(new Point(OffSetX, OffSetY), p, 2))
                return Functions.DirectionFromPoint(new Point(OffSetX, OffSetY), p);

            PointF c = new PointF(OffSetX * CellWidth + CellWidth / 2F + ViewOffsetX, OffSetY * CellHeight + CellHeight / 2F + ViewOffsetY);
        ...
        {
            int viewWidth = ViewPixelWidth;
            int viewHeight = ViewPixelHeight;
            ...
            foreach (WeatherSetting itemWeather in Enum.GetValues(typeof(WeatherSetting)).Cast<object>().ToArray())
            {
                ...
                switch (itemWeather)
                {
                    case WeatherSetting.黄色花瓣:
                        ...
                        for (int y = 512 * -1; y < viewHeight + 512; y += 512)
                            for (int x = 512 * -1; x < viewWidth + 512; x += 512)
                            {
                    ...
                    case WeatherSetting.雨天:
                        ...
                        var xVar = 512;
                        var yVar = 512;
                        for (int y = yVar * -1; y < viewHeight + yVar; y += yVar)
                            for (int x = xVar * -1; x < viewWidth + xVar; x += xVar)
                            {
                    ...
                    case WeatherSetting.落雪:
                        ...
                        for (int y = ySnowfallVar * -1; y < viewHeight + ySnowfallVar; y += ySnowfallVar)
                            for (int x = xSnowfallVar * -1; x < viewWidth + xSnowfallVar; x += xSnowfallVar)
                            {
                    ...
                    case WeatherSetting.飘雪:
                        ...
                        for (int y = -400; y < viewHeight + 400; y += 400)
                            for (int x = -400; x < viewWidth + 400; x += 400)
                            {
                    ...
                    case WeatherSetting.雾天:
                        ...
                        for (int y = -512; y < viewHeight + 512; y += 512)
                            for (int x = -512; x < viewWidth + 512; x += 512)
                            {
