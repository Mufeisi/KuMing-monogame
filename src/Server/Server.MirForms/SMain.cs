...
namespace Server
{
    ...
    {
        private ToolStripMenuItem? _scriptDebugToolStripMenuItem;
        ...
        public SMain()
        {
            ...
            InitializeScriptDebugMenu();
            AutoResize();
        }

        private void InitializeScriptDebugMenu()
        {
            if (_scriptDebugToolStripMenuItem != null) return;

            _scriptDebugToolStripMenuItem = new ToolStripMenuItem
            {
                Name = "scriptDebugToolStripMenuItem",
                Text = "脚本调试...",
            };

            _scriptDebugToolStripMenuItem.Click += scriptDebugToolStripMenuItem_Click;

            var insertIndex = MainMenu.Items.IndexOf(CharacterToolStripMenuItem);
            if (insertIndex < 0) insertIndex = MainMenu.Items.IndexOf(UpTimeLabel);
            if (insertIndex < 0) insertIndex = MainMenu.Items.Count;

            MainMenu.Items.Insert(insertIndex, _scriptDebugToolStripMenuItem);
        }
        ...
        {
            if (Envir.Running && SqlSaveResilience.ShouldBlockShutdown(out var reason))
            {
                var result = MessageBox.Show(
                    reason + Environment.NewLine + Environment.NewLine + "是否强制关服？",
                    "停服保护",
        ...
        private void scriptDebugToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var warning = "脚本调试包含脚本编辑、历史回滚、编译热更等高危操作。\r\n建议仅在测试服或维护窗口使用。\r\n\r\n是否继续？";
