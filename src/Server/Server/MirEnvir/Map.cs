...
namespace Server.MirEnvir
{
    public class Map
    {
        ...
        private void ProcessRespawns()
        {
            for (int i = 0; i < Respawns.Count; i++)
            {
                ...
                var allowCSharpBefore = scriptsRuntimeActive && Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMonsterRespawnBefore);
                var allowCSharpAfter = scriptsRuntimeActive && Server.Scripting.ScriptDispatchPolicy.ShouldTryCSharp(Server.Scripting.ScriptHookKeys.OnMonsterRespawnAfter);

                var desiredCount = respawn.Info.Count * Envir.SpawnMultiplier;
                var spawnCount = desiredCount - respawn.Count;
                if (spawnCount < 0) spawnCount = 0;

                var delayMinutes = (int)respawn.Info.Delay;
                var randomDelayMinutes = (int)respawn.Info.RandomDelay;
                var useRespawnTicks = respawn.Info.RespawnTicks != 0;
                var respawnTicks = (int)respawn.Info.RespawnTicks;

                var scriptDecision = Server.Scripting.ScriptHookDecision.Continue;

                if (allowCSharpBefore)
                {
                    var request = new Server.Scripting.MonsterRespawnRequest(
                        this,
                        respawn,
                        respawn.Monster,
                        Envir.SpawnMultiplier,
                        Envir.Time,
                        Envir.RespawnTick.CurrentTickcounter,
                        desiredCount,
                        spawnCount,
                        delayMinutes,
                        randomDelayMinutes,
                    ...
                    Envir.CSharpScripts.TryHandleMonsterRespawnBefore(respawn.Monster, request);
                    ...
                    randomDelayMinutes = request.RandomDelayMinutes;
                    useRespawnTicks = request.UseRespawnTicks;
                    respawnTicks = request.RespawnTicks;
                }
                else if (scriptsRuntimeActive && Settings.TxtScriptsLogDispatch)
                {
                    MessageQueue.Enqueue($"[Scripts][Dispatch] MonsterRespawnBefore {respawn.Monster?.Index}:{respawn.Monster?.Name} map={Info.Index} respawn={respawn.Info.RespawnIndex} -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnMonsterRespawnBefore}）");
                }

                if (desiredCount < 0) desiredCount = 0;
                if (spawnCount < 0) spawnCount = 0;
                if (delayMinutes < 1) delayMinutes = 1;
                if (randomDelayMinutes < 0) randomDelayMinutes = 0;
                if (useRespawnTicks && respawnTicks < 1) respawnTicks = 1;

                var executedLegacy = false;
                var success = true;

                var countBefore = respawn.Count;
                var errorCountBefore = respawn.ErrorCount;

                if (scriptDecision == Server.Scripting.ScriptHookDecision.Cancel)
                {
                    // 取消本次刷新（仍按成功逻辑推进下一次刷新时间，以避免频繁触发）。
                    success = true;
                }
                ...
                else
                {
                    ...
                    {
                        var ok = respawn.Spawn();
                        if (!ok) success = false;
                    }
                }

                var spawnedCount = respawn.Count - countBefore;
                ...
                if (success)
                {
                    ...
                    {
                        delay = delayMinutes - randomDelayMinutes + Envir.Random.Next(randomDelayMinutes * 2);
                    }
                    ...
                    if (useRespawnTicks)
                    {
                        respawn.NextSpawnTick = Envir.RespawnTick.CurrentTickcounter + (ulong)respawnTicks;
                ...
                else
                {
                    ...
                    if (useRespawnTicks)
                    {
                        respawn.NextSpawnTick = Envir.RespawnTick.CurrentTickcounter + 1;
                        if (respawn.NextSpawnTick > long.MaxValue)
                            respawn.NextSpawnTick -= long.MaxValue;
                    }
                ...
                if (allowCSharpAfter)
                {
                    var result = new Server.Scripting.MonsterRespawnResult(
                        this,
                        respawn,
                        respawn.Monster,
                        Envir.SpawnMultiplier,
                    ...
                    Envir.CSharpScripts.TryHandleMonsterRespawnAfter(respawn.Monster, result);
                }
                else if (scriptsRuntimeActive && Settings.TxtScriptsLogDispatch)
                {
                    MessageQueue.Enqueue($"[Scripts][Dispatch] MonsterRespawnAfter {respawn.Monster?.Index}:{respawn.Monster?.Name} map={Info.Index} respawn={respawn.Info.RespawnIndex} -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnMonsterRespawnAfter}）");
                }
            }
        }
    ...
    public class MapRespawn
    {
        ...
        public void LoadRoutes()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(Info.RoutePath))
                {
                    Route = new List<RouteInfo>();
                    return;
                }

                Route = Envir.LoadRouteDefinitionOrEmpty(Info.RoutePath, Info.RespawnIndex, Info.MonsterIndex);
            }
            catch (Exception ex)
            {
                MessageQueue.Instance.Enqueue("[Scripts] Routes 加载失败：" + ex);
                Route = new List<RouteInfo>();
            }
        }
    }
