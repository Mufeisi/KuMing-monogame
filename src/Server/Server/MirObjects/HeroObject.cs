...
namespace Server.MirObjects
{
    public class HeroObject : HumanObject
    {
        ...
        public override void UseItem(ulong id)
        {
            ...
            {
                MessageQueue.Enqueue($"[Scripts][Dispatch] HeroUseItem -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnPlayerItemUseCheck}）");
            }
            ...
            if (allowCSharpCheck && Owner != null && item != null && index != -1)
            {
                var req = new Server.Scripting.PlayerItemUseCheckRequest(MirGridType.HeroInventory, item, index, Dead);
                Envir.CSharpScripts.TryHandlePlayerItemUseCheck(Owner, req);

                if (req.Decision == Server.Scripting.ScriptHookDecision.Cancel)
                {
                    if (!string.IsNullOrEmpty(req.FailMessage))
                        Owner.ReceiveChat(req.FailMessage, ChatType.System);
