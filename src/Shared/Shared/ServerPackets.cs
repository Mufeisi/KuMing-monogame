...
namespace ServerPackets
{
    ...
    public sealed class Fg : Packet
    {
        public override short Index
        {
            get { return (short)ServerPacketIds.Fg; }
        }

        public FgType Type;
        public string FgIPAddress;
        public ushort FgPort;
        protected override void ReadPacket(BinaryReader reader)
        {
            Type = (FgType)reader.ReadByte();
            FgIPAddress = reader.ReadString();
            FgPort = reader.ReadUInt16();
        }

        protected override void WritePacket(BinaryWriter writer)
        {
            writer.Write((byte)Type);
            writer.Write(FgIPAddress);
            writer.Write(FgPort);
        }
    }
