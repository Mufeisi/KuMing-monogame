...
public static class PlayerArtifactBuilder
{
    ...
    public static PlayerPayloadInfo Create(EditorProject project, string projectRoot, string outputExe, string? microCode)
    {
        ...
        try
        {
            ...
            ExtractTemplate(payloadResource, payload);
            string builtIn = Path.Combine(payload, "Launcher", "BuiltIn"); Directory.CreateDirectory(builtIn);
            CopyProjectAssets(projectRoot, builtIn, project.Snapshot);
    ...
    private static void ExtractTemplate(Stream source, string destination)
    {
        ...
        foreach (ZipArchiveEntry entry in archive.Entries)
        {
            ...
            using Stream input = entry.Open(); using var output = new FileStream(target, FileMode.CreateNew, FileAccess.Write, FileShare.None); input.CopyTo(output);
        }
    }

    private static void CopyProjectAssets(string projectRoot, string builtIn, LauncherSnapshot snapshot)
    {
